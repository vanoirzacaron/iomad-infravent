...
/**
 * peerreview module external functions tests
 *
...
class external_test extends externallib_advanced_testcase {
    ...
    private $course;
    /** @var stdClass peerreview object */
    private $peerreview;
    ...
    public function setUp(): void {
        ...
        $this->peerreview = $this->getDataGenerator()->create_module('peerreview',
            array(
                'course' => $this->course->id,
                'overallfeedbackfiles' => 1,
            )
        );
        $this->context = \context_module::instance($this->peerreview->cmid);
        $this->cm = get_coursemodule_from_instance('peerreview', $this->peerreview->id);

        // Add grading strategy data (accumulative is the default).
        $peerreview = new peerreview($this->peerreview, $this->cm, $this->course);
        ...
        $data['peerreviewid'] = $peerreview->id;
        $data['norepeats'] = 4;
    ...
    /**
     * Test test_mod_peerreview_get_peerreviews_by_courses
     */
    public function test_mod_peerreview_get_peerreviews_by_courses() {
        ...
        $peerreview2 = self::getDataGenerator()->create_module('peerreview', $record);
        ...
        $returndescription = mod_peerreview_external::get_peerreviews_by_courses_returns();

        // Create what we expect to be returned when querying the two courses.
        $properties = peerreview_summary_exporter::read_properties_definition();
        $expectedfields = array_keys($properties);

        // Add expected coursemodule and data.
        $peerreview1 = $this->peerreview;
        $peerreview1->coursemodule = $peerreview1->cmid;
        $peerreview1->introformat = 1;
        $peerreview1->introfiles = [];
        $peerreview1->lang = '';
        $peerreview1->instructauthorsfiles = [];
        $peerreview1->instructauthorsformat = 1;
        $peerreview1->instructreviewersfiles = [];
        $peerreview1->instructreviewersformat = 1;
        $peerreview1->conclusionfiles = [];
        $peerreview1->conclusionformat = 1;
        $peerreview1->submissiontypetext = 1;
        $peerreview1->submissiontypefile = 1;

        $peerreview2->coursemodule = $peerreview2->cmid;
        $peerreview2->introformat = 1;
        $peerreview2->introfiles = [];
        $peerreview2->lang = '';
        $peerreview2->instructauthorsfiles = [];
        $peerreview2->instructauthorsformat = 1;
        $peerreview2->instructreviewersfiles = [];
        $peerreview2->instructreviewersformat = 1;
        $peerreview2->conclusionfiles = [];
        $peerreview2->conclusionformat = 1;
        $peerreview2->submissiontypetext = 1;
        $peerreview2->submissiontypefile = 1;

        foreach ($expectedfields as $field) {
            if (!empty($properties[$field]) && $properties[$field]['type'] == PARAM_BOOL) {
                $peerreview1->{$field} = (bool) $peerreview1->{$field};
                $peerreview2->{$field} = (bool) $peerreview2->{$field};
            }
            $expected1[$field] = $peerreview1->{$field};
            $expected2[$field] = $peerreview2->{$field};
        }

        $expectedpeerreviews = array($expected2, $expected1);

        // Call the external function passing course ids.
        $result = mod_peerreview_external::get_peerreviews_by_courses(array($course2->id, $this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);

        $this->assertEquals($expectedpeerreviews, $result['peerreviews']);
        $this->assertCount(0, $result['warnings']);

        // Call the external function without passing course id.
        $result = mod_peerreview_external::get_peerreviews_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedpeerreviews, $result['peerreviews']);
        $this->assertCount(0, $result['warnings']);

        // Unenrol user from second course and alter expected peerreviews.
        $enrol->unenrol_user($instance2, $this->student->id);
        array_shift($expectedpeerreviews);
        ...
        $result = mod_peerreview_external::get_peerreviews_by_courses(array($course2->id));
    ...
    /**
     * Test mod_peerreview_get_peerreview_access_information for students.
     */
    public function test_mod_peerreview_get_peerreview_access_information_student() {

        self::setUser($this->student);
        $result = mod_peerreview_external::get_peerreview_access_information($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_peerreview_access_information_returns(), $result);
        ...
        unassign_capability('mod/peerreview:peerassess', $this->studentrole->id);
        unassign_capability('mod/peerreview:submit', $this->studentrole->id);
        unset($enabledcaps[0]);
        unset($enabledcaps[1]);
        ...
        // Switch phase.
        $peerreview = new peerreview($this->peerreview, $this->cm, $this->course);
        $peerreview->switch_phase(peerreview::PHASE_SUBMISSION);
        $result = mod_peerreview_external::get_peerreview_access_information($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_peerreview_access_information_returns(), $result);
        ...
        $peerreview->switch_phase(peerreview::PHASE_ASSESSMENT);
        $result = mod_peerreview_external::get_peerreview_access_information($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_peerreview_access_information_returns(), $result);
    ...
    /**
     * Test mod_peerreview_get_peerreview_access_information for teachers.
     */
    public function test_mod_peerreview_get_peerreview_access_information_teacher() {
    ...
    /**
     * Test mod_peerreview_get_user_plan for students.
     */
    public function test_mod_peerreview_get_user_plan_student() {

        self::setUser($this->student);
        $result = mod_peerreview_external::get_user_plan($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_user_plan_returns(), $result);

        $this->assertCount(0, $result['userplan']['examples']);  // No examples given.
        $this->assertCount(5, $result['userplan']['phases']);  // Always 5 phases.
        $this->assertEquals(peerreview::PHASE_SETUP, $result['userplan']['phases'][0]['code']);  // First phase always setup.
        $this->assertTrue($result['userplan']['phases'][0]['active']); // First phase "Setup" active in new peerreviews.
        ...
        $this->assertEquals(peerreview::PHASE_SUBMISSION, $result['userplan']['phases'][1]['code']);
        $this->assertTrue($result['userplan']['phases'][1]['active']); // We are now in submission phase.
    }

    /**
     * Test mod_peerreview_get_user_plan for teachers.
     */
    public function test_mod_peerreview_get_user_plan_teacher() {
        ...
        $this->assertEquals(peerreview::PHASE_SETUP, $result['userplan']['phases'][0]['code']);  // First phase always setup.
        $this->assertTrue($result['userplan']['phases'][0]['active']); // First phase "Setup" active in new peerreviews.
        $this->assertCount(4, $result['userplan']['phases'][0]['tasks']);  // For new empty peerreviews, always 4 tasks.
    ...
    /**
     * Test test_view_peerreview invalid id.
     */
    public function test_view_peerreview_invalid_id() {
        $this->expectException('moodle_exception');
        mod_peerreview_external::view_peerreview(0);
    }

    /**
     * Test test_view_peerreview user not enrolled.
     */
    public function test_view_peerreview_user_not_enrolled() {
        // Test not-enrolled user.
        $usernotenrolled = self::getDataGenerator()->create_user();
        $this->setUser($usernotenrolled);
        $this->expectException('moodle_exception');
        mod_peerreview_external::view_peerreview($this->peerreview->id);
    }

    /**
     * Test test_view_peerreview user student.
     */
    public function test_view_peerreview_user_student() {
        ...
        $result = mod_peerreview_external::view_peerreview($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::view_peerreview_returns(), $result);
        ...
        $this->assertInstanceOf('\mod_peerreview\event\course_module_viewed', $event);
        $this->assertEquals($this->context, $event->get_context());
        $moodlepeerreview = new \moodle_url('/mod/peerreview/view.php', array('id' => $this->cm->id));
        $this->assertEquals($moodlepeerreview, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    }

    /**
     * Test test_view_peerreview user missing capabilities.
     */
    public function test_view_peerreview_user_missing_capabilities() {
        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/peerreview:view', CAP_PROHIBIT, $this->studentrole->id, $this->context->id);
    ...
    public function test_add_submission() {
        ...
        $result = mod_peerreview_external::add_submission($this->peerreview->id, $title, $content, FORMAT_MOODLE, $draftidinlineattach,
            $draftidattach);
        $result = external_api::clean_returnvalue(mod_peerreview_external::add_submission_returns(), $result);
        $this->assertEmpty($result['warnings']);

        // Check submission created.
        $submission = $peerreview->get_submission_by_author($this->student->id);
        $this->assertTrue($result['status']);
        ...
        // Check files.
        $contentfiles = $fs->get_area_files($this->context->id, 'mod_peerreview', 'submission_content', $submission->id);
        ...
        $contentfiles = $fs->get_area_files($this->context->id, 'mod_peerreview', 'submission_attachment', $submission->id);
        $this->assertCount(2, $contentfiles);
        foreach ($contentfiles as $file) {
            if ($file->is_directory()) {
                continue;
    ...
    public function test_add_submission_invalid_phase() {
        ...
        $this->expectException('moodle_exception');
        mod_peerreview_external::add_submission($this->peerreview->id, 'Test');
    }
    ...
    public function test_add_submission_empty_title() {
        ...
        $this->expectException('moodle_exception');
        mod_peerreview_external::add_submission($this->peerreview->id, '');
    }
    ...
    public function test_add_submission_already_added() {
        ...
        $result = mod_peerreview_external::add_submission($this->peerreview->id, 'My submission', '', FORMAT_MOODLE, 0, $draftidattach);
        $result = external_api::clean_returnvalue(mod_peerreview_external::add_submission_returns(), $result);
    ...
    public function test_update_submission() {
        ...
        $result = mod_peerreview_external::update_submission($submissionid, $title, $content, FORMAT_MOODLE, $draftidinlineattach,
            $draftidattach);
        $result = external_api::clean_returnvalue(mod_peerreview_external::update_submission_returns(), $result);
        $this->assertEmpty($result['warnings']);

        // Check submission updated.
        $peerreview = new peerreview($this->peerreview, $this->cm, $this->course);
        $submission = $peerreview->get_submission_by_id($submissionid);
        $this->assertTrue($result['status']);
        $this->assertEquals($title, $submission->title);
        $this->assertEquals($content, $submission->content);
    ...
    public function test_update_submission_of_other_user() {
        ...
        $this->expectException('moodle_exception');
        mod_peerreview_external::update_submission($submissionid, 'Test');
    }
    ...
    public function test_update_submission_empty_title() {
        ...
        $this->expectException('moodle_exception');
        mod_peerreview_external::update_submission($submissionid, '');
    }
    ...
    public function test_delete_submission() {
        ...
        $result = mod_peerreview_external::delete_submission($submissionid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::delete_submission_returns(), $result);
        $this->assertEmpty($result['warnings']);
        $this->assertTrue($result['status']);
        $peerreview = new peerreview($this->peerreview, $this->cm, $this->course);
        $submission = $peerreview->get_submission_by_author($this->student->id);
        ...
        // Checking event.
        $this->assertInstanceOf('\mod_peerreview\event\submission_deleted', $event);
        $this->assertEquals($this->context, $event->get_context());
    }
    ...
    public function test_delete_submission_with_assessments() {
        ...
        $peerreviewgenerator->create_assessment($submissionid, $this->teacher->id, array(
            'weight' => 3,
            'grade' => 95.00000,
        ));
        ...
        $this->setUser($this->student);
        $this->expectException('moodle_exception');
        mod_peerreview_external::delete_submission($submissionid);
    }
    ...
    public function test_get_submissions_student() {
        ...
        $this->setUser($this->student);
        $result = mod_peerreview_external::get_submissions($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submissions_returns(), $result);
    ...
    public function test_get_submissions_published_student() {
        ...
        $peerreview = new peerreview($this->peerreview, $this->cm, $this->course);
        $peerreview->switch_phase(peerreview::PHASE_CLOSED);
        // Create a couple of submissions with files.
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $submission = array('published' => 1);
        $submissionid = $peerreviewgenerator->create_submission($this->peerreview->id, $this->anotherstudentg1->id, $submission);
    ...
    public function test_get_submissions_from_student_with_feedback_from_teacher() {
        ...
        // Create a couple of submissions with files.
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $submissionid = $peerreviewgenerator->create_submission($this->peerreview->id, $this->student->id);
        ...
        // Remove teacher caps.
        assign_capability('mod/peerreview:viewallsubmissions', CAP_PROHIBIT, $this->teacher->id, $this->context->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
        \course_modinfo::clear_instance_cache();

        $this->setUser($this->teacher);
        $result = mod_peerreview_external::get_submissions($this->peerreview->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submissions_returns(), $result);
        // We should get just our submission.
        $this->assertEquals(1, $result['totalcount']);
        $this->assertEquals($submissionid, $result['submissions'][0]['id']);
    ...
    public function test_get_submissions_from_students_as_teacher() {
        ...
        // Create a couple of submissions with files.
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $submissionid1 = $peerreviewgenerator->create_submission($this->peerreview->id, $this->student->id);
        $submissionid2 = $peerreviewgenerator->create_submission($this->peerreview->id, $this->anotherstudentg1->id);
        $submissionid3 = $peerreviewgenerator->create_submission($this->peerreview->id, $this->anotherstudentg2->id);

        $this->setUser($this->teacher);
        $result = mod_peerreview_external::get_submissions($this->peerreview->id); // Get all.
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submissions_returns(), $result);
        $this->assertEquals(3, $result['totalcount']);
        $this->assertCount(3, $result['submissions']);

        $result = mod_peerreview_external::get_submissions($this->peerreview->id, 0, 0, 0, 2); // Check pagination.
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submissions_returns(), $result);
        $this->assertEquals(3, $result['totalcount']);
        $this->assertCount(2, $result['submissions']);

        $result = mod_peerreview_external::get_submissions($this->peerreview->id, 0, $this->group2->id); // Get group 2.
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submissions_returns(), $result);
        $this->assertEquals(1, $result['totalcount']);
        $this->assertCount(1, $result['submissions']);
        $this->assertEquals($submissionid3, $result['submissions'][0]['id']);

        $result = mod_peerreview_external::get_submissions($this->peerreview->id, $this->anotherstudentg1->id); // Get one.
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submissions_returns(), $result);
        $this->assertEquals(1, $result['totalcount']);
    ...
    public function test_get_submission_student() {
        ...
        $peerreview->switch_phase(peerreview::PHASE_CLOSED);
        $this->setUser($this->student);
        $result = mod_peerreview_external::get_submission($firstsubmissionid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submission_returns(), $result);
        ...
        $peerreview->switch_phase(peerreview::PHASE_EVALUATION);
        $result = mod_peerreview_external::get_submission($firstsubmissionid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submission_returns(), $result);
        $this->assertEquals($firstsubmissionid, $result['submission']['id']);
        $this->assertCount(1, $result['submission']['contentfiles']); // Check we retrieve submission text files.
    ...
    public function test_get_submission_i_reviewed() {
        ...
        $firstsubmissionid = $this->create_test_submission($this->student);  // Create submission with files.
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $peerreviewgenerator->create_assessment($firstsubmissionid, $this->anotherstudentg1->id, array(
            'weight' => 3,
            'grade' => 95,
        ));
    ...
    public function test_get_submission_published_student() {
        ...
        $this->setUser($this->student);
        $result = mod_peerreview_external::get_submission($submissionid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submission_returns(), $result);
        ...
        $this->setUser($this->anotherstudentg2);
        $this->expectException('moodle_exception');
        mod_peerreview_external::get_submission($submissionid);
    }
    ...
    public function test_get_submission_from_student_with_feedback_from_teacher() {
        ...
        $peerreview->switch_phase(peerreview::PHASE_EVALUATION);
        unassign_capability('mod/peerreview:viewallsubmissions', $this->teacherrole->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
    ...
    public function test_get_submission_from_students_as_teacher() {
        ...
        $this->setUser($this->teacher);
        $result = mod_peerreview_external::get_submission($submissionid1); // Get all.
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submission_returns(), $result);
        $this->assertEquals($submissionid1, $result['submission']['id']);

        $result = mod_peerreview_external::get_submission($submissionid3); // Get group 2.
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submission_returns(), $result);
    ...
    public function test_get_submission_assessments_student() {
        ...
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $peerreviewgenerator->create_assessment($submissionid, $this->anotherstudentg1->id, array(
            'weight' => 3,
            'grade' => 95,
        ));
        $peerreviewgenerator->create_assessment($submissionid, $this->student->id, array(
            'weight' => 2,
            'grade' => 90,
        ));
        ...
        $peerreview->switch_phase(peerreview::PHASE_CLOSED);
        $this->setUser($this->student);
        $result = mod_peerreview_external::get_submission_assessments($submissionid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_submission_assessments_returns(), $result);
    ...
    public function test_get_submission_assessments_invalid_phase() {
        ...
        $this->expectException('moodle_exception');
        mod_peerreview_external::get_submission_assessments($submissionid);
    }
    ...
    public function test_get_submission_assessments_teacher() {
        ...
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $assessmentid = $peerreviewgenerator->create_assessment($submissionid, $this->anotherstudentg1->id, array(
            'weight' => 1,
            'grade' => 50,
        ));
    ...
    public function test_get_assessment_author() {
        ...
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $assessmentid = $peerreviewgenerator->create_assessment($submissionid, $this->student->id, array(
            'weight' => 2,
            'grade' => 90,
        ));
        ...
        $peerreview->switch_phase(peerreview::PHASE_CLOSED);
        $this->setUser($this->anotherstudentg1);
        $result = mod_peerreview_external::get_assessment($assessmentid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_assessment_returns(), $result);
    ...
    public function test_get_assessment_student_invalid_phase() {
        ...
        $this->expectException('moodle_exception');
        mod_peerreview_external::get_assessment($assessmentid);
    }
    ...
    public function test_get_assessment_form_definition_reviewer_new_assessment() {
        ...
        $peerreview = new peerreview($this->peerreview, $this->cm, $this->course);
        $submission = $peerreview->get_submission_by_id($submissionid);
        $assessmentid = $peerreview->add_allocation($submission, $this->student->id);

        // Switch to assessment phase.
        $peerreview->switch_phase(peerreview::PHASE_ASSESSMENT);
        $this->setUser($this->student);
        $result = mod_peerreview_external::get_assessment_form_definition($assessmentid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_assessment_form_definition_returns(), $result);
    ...
    public function test_get_assessment_form_definition_invalid_phase() {
        ...
        $peerreview = new peerreview($this->peerreview, $this->cm, $this->course);
        $submission = $peerreview->get_submission_by_id($submissionid);
        $assessmentid = $peerreview->add_allocation($submission, $this->anotherstudentg1->id);

        $peerreview->switch_phase(peerreview::PHASE_EVALUATION);
        $this->setUser($this->student);
        // Since we are not reviewers we can't see the assessment until the peerreview is closed.
        $this->expectException('moodle_exception');
        mod_peerreview_external::get_assessment_form_definition($assessmentid);
    }
    ...
    public function test_get_reviewer_assessments() {
        ...
        $peerreviewgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peerreview');
        $assessmentid1 = $peerreviewgenerator->create_assessment($submissionid1, $this->student->id, array(
            'weight' => 2,
            'grade' => 90,
        ));
        $assessmentid2 = $peerreviewgenerator->create_assessment($submissionid2, $this->student->id, array(
            'weight' => 3,
            'grade' => 80,
        ));
        ...
        $this->setUser($this->student);
        // Get my assessments.
        $result = mod_peerreview_external::get_reviewer_assessments($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_reviewer_assessments_returns(), $result);
        ...
        $result = mod_peerreview_external::get_reviewer_assessments($this->peerreview->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_reviewer_assessments_returns(), $result);
        $this->assertCount(2, $result['assessments']);
    ...
    public function test_get_reviewer_assessments_other_student() {
        ...
        $this->setUser($this->student);
        $this->expectException('moodle_exception');
        mod_peerreview_external::get_reviewer_assessments($this->peerreview->id, $this->anotherstudentg1->id);
    }
    ...
    public function test_update_assessment() {
        ...
        $result = mod_peerreview_external::update_assessment($assessmentid, $data);
        $result = external_api::clean_returnvalue(mod_peerreview_external::update_assessment_returns(), $result);
    ...
    public function test_get_grades() {
        ...
        peerreview_grade_item_update($this->peerreview, (object) $submissiongrade, (object) $assessmentgrade);

        // First retrieve my grades.
        $this->setUser($this->student);
        $result = mod_peerreview_external::get_grades($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_grades_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        ...
        $this->setUser($this->teacher);
        $result = mod_peerreview_external::get_grades($this->peerreview->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_grades_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertEquals($assessmentgrade['rawgrade'], $result['assessmentrawgrade']);
        $this->assertEquals($submissiongrade['rawgrade'], $result['submissionrawgrade']);
    ...
    public function test_get_grades_other_student() {
        ...
        $this->setUser($this->anotherstudentg1);
        $this->expectException('moodle_exception');
        mod_peerreview_external::get_grades($this->peerreview->id, $this->student->id);
    }
    ...
    public function test_evaluate_assessment() {
        ...
        $weight = 10;
        $gradinggradeover = 10;
        $result = mod_peerreview_external::evaluate_assessment($assessmentid, $feedbacktext, $feedbackformat, $weight,
            $gradinggradeover);
        $result = external_api::clean_returnvalue(mod_peerreview_external::evaluate_assessment_returns(), $result);
        $this->assertTrue($result['status']);

        $assessment = $DB->get_record('peerreview_assessments', array('id' => $assessmentid));
    ...
    public function test_evaluate_assessment_ignore_parameters() {
        ...
        assign_capability('mod/peerreview:allocate', CAP_PROHIBIT, $this->teacherrole->id, $this->context->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
    ...
    public function test_evaluate_assessment_no_permissions() {
        ...
        $gradinggradeover = 50;
        $this->expectException('moodle_exception');
        mod_peerreview_external::evaluate_assessment($assessmentid, $feedbacktext, $feedbackformat, $weight, $gradinggradeover);
    }
    ...
    public function test_get_grades_report() {
        ...
        $assessmentid1 = $peerreviewgenerator->create_assessment($submissionid2, $this->student->id, array(
            'weight' => 100,
            'grade' => 50,
        ));
        $assessmentid2 = $peerreviewgenerator->create_assessment($submissionid1, $this->anotherstudentg1->id, array(
            'weight' => 100,
            'grade' => 55,
        ));
        ...
        $peerreview->switch_phase(peerreview::PHASE_CLOSED);
        $this->setUser($this->teacher);
        $result = mod_peerreview_external::get_grades_report($this->peerreview->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_grades_report_returns(), $result);
        ...
        foreach ($result['report']['grades'] as $grade) {
            if ($grade['userid'] == $this->student->id) {
                ...
                $this->assertEquals($peerreview->real_grade(50), $grade['reviewerof'][0]['grade']); // Check grade (converted).
                $this->assertEquals($peerreview->real_grade(55), $grade['reviewedby'][0]['grade']); // Check grade (converted).
                ...
                $this->assertEquals($peerreview->real_grade(55), $grade['reviewerof'][0]['grade']); // Check grade (converted).
                $this->assertEquals($peerreview->real_grade(50), $grade['reviewedby'][0]['grade']); // Check grade (converted).
            }
        }
        // Now check pagination.
        $result = mod_peerreview_external::get_grades_report($this->peerreview->id, 0, 'lastname', 'ASC', 0, 1);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_grades_report_returns(), $result);
        $this->assertEquals(3, $result['report']['totalcount']); // Expect the total count.
        $this->assertCount(1, $result['report']['grades']);

        // Groups filtering.
        $result = mod_peerreview_external::get_grades_report($this->peerreview->id, $this->group1->id);
        $result = external_api::clean_returnvalue(mod_peerreview_external::get_grades_report_returns(), $result);
    ...
    public function test_get_grades_report_invalid_phase() {
        $this->setUser($this->teacher);
        $this->expectException('moodle_exception');
        $this->expectExceptionMessage(get_string('nothingfound', 'peerreview'));
        mod_peerreview_external::get_grades_report($this->peerreview->id);
    }
    ...
    public function test_view_submission() {
        ...
        $this->setUser($this->student);
        $result = mod_peerreview_external::view_submission($firstsubmissionid);
        $result = external_api::clean_returnvalue(mod_peerreview_external::view_submission_returns(), $result);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_peerreview\event\submission_viewed', $event);
        $this->assertEquals($this->context, $event->get_context());
        $moodlepeerreview = new \moodle_url('/mod/peerreview/submission.php', array('id' => $firstsubmissionid,
            'cmid' => $this->cm->id));
        $this->assertEquals($moodlepeerreview, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    ...
    public function test_evaluate_submission() {
        ...
        $published = 1;
        $gradeover = 10;
        $result = mod_peerreview_external::evaluate_submission($submissionid, $feedbacktext, $feedbackformat, $published,
            $gradeover);
        $result = external_api::clean_returnvalue(mod_peerreview_external::evaluate_submission_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals($peerreview->raw_grade_value($gradeover, $peerreview->grade), $submission->gradeover);  // Expected grade.
    ...
    public function test_evaluate_submission_no_permissions() {
        ...
        $gradeover = 50;
        $this->expectException('moodle_exception');
        mod_peerreview_external::evaluate_submission($submissionid, $feedbacktext, $feedbackformat, $published, $gradeover);
    }
    ...
    public function test_evaluate_submission_invalid_grade() {
        ...
        $published = 1;
        $gradeover = 150;
        $result = mod_peerreview_external::evaluate_submission($submissionid, $feedbacktext, $feedbackformat, $published, $gradeover);
        $result = external_api::clean_returnvalue(mod_peerreview_external::evaluate_submission_returns(), $result);
        $this->assertCount(1, $result['warnings']);
        $this->assertFalse($result['status']);
