 ...
 */
class peerreview_best_evaluation extends peerreview_evaluation {

    /** @var the recently used settings in this peerreview */
    protected $settings;

    /**
     * Constructor
     *
     * @param peerreview $peerreview The peerreview api instance
     * @return void
     */
    public function __construct(peerreview $peerreview) {
        global $DB;
        $this->peerreview = $peerreview;
        $this->settings = $DB->get_record('peerrevieweval_best_settings', array('peerreviewid' => $this->peerreview->id));
    }

    /**
     * Calculates the grades for assessment and updates 'gradinggrade' fields in 'peerreview_assessments' table
     *
    ...
    public function update_grading_grades(stdclass $settings, $restrict=null) {
        ...
        // Remember the recently used settings for this peerreview.
        if (empty($this->settings)) {
            $record = new stdclass();
            $record->peerreviewid = $this->peerreview->id;
            $record->comparison = $settings->comparison;
            $DB->insert_record('peerrevieweval_best_settings', $record);
        } elseif ($this->settings->comparison != $settings->comparison) {
            $DB->set_field('peerrevieweval_best_settings', 'comparison', $settings->comparison,
                    array('peerreviewid' => $this->peerreview->id));
        }

        // Get the grading strategy instance.
        $grader = $this->peerreview->grading_strategy_instance();
     ...
     *
     * @return peerreview_best_evaluation_settings_form
     */
    public function get_settings_form(moodle_url $actionurl=null) {
        ...
        return new peerreview_best_evaluation_settings_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public static function delete_instance($peerreviewid) {
        ...
        $DB->delete_records('peerrevieweval_best_settings', array('peerreviewid' => $peerreviewid));
    }
    ...
    protected function process_assessments(array $assessments, array $diminfo, stdclass $settings) {
        ...
        if (is_null($average)) {
            foreach ($assessments as $asid => $assessment) {
                if (!is_null($assessment->gradinggrade)) {
                    $DB->set_field('peerreview_assessments', 'gradinggrade', null, array('id' => $asid));
                }
            }
            return;
        }
        ...
        foreach ($grades as $assessmentid => $grade) {
            if (grade_floats_different($grade, $assessments[$assessmentid]->gradinggrade)) {
                ...
                $DB->update_record('peerreview_assessments', $record, true);  // bulk operations expected
            }
        }
 ...
 */
class peerreview_best_evaluation_settings_form extends peerreview_evaluation_settings_form {
    ...
    protected function definition_sub() {
        ...
        $plugindefaults = get_config('peerrevieweval_best');
        $current = $this->_customdata['current'];

        $options = array();
        for ($i = 9; $i >= 1; $i = $i-2) {
            $options[$i] = get_string('comparisonlevel' . $i, 'peerrevieweval_best');
        }
        $mform->addElement('select', 'comparison', get_string('comparison', 'peerrevieweval_best'), $options);
        $mform->addHelpButton('comparison', 'comparison', 'peerrevieweval_best');
