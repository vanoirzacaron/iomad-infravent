...
/**
 * Class for exporting partial peerreview data.
 *
...
/**
 * Class for exporting partial peerreview data (some fields are only viewable by admins).
 *
 * @copyright  2017 Juan Leyva <juan@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class peerreview_summary_exporter extends exporter {
    ...
    protected static function define_properties() {
        ...
        return array(
            ...
            'course' => array(
                'type' => PARAM_INT,
                'description' => 'Course id this peerreview is part of.',
            ),
            'name' => array(
                'type' => PARAM_TEXT,
                'description' => 'peerreview name.',
            ),
            'intro' => array(
                'default' => '',
                'type' => PARAM_RAW,
                'description' => 'peerreview introduction text.',
                'null' => NULL_ALLOWED,
            ),
            'introformat' => array(
                ...
                'description' => 'peerreview intro text format.',
            ),
            'lang' => array(
                'type' => PARAM_LANG,
            ...
            'phase' => array(
                'type' => PARAM_INT,
                'default' => 0,
                'description' => 'The current phase of peerreview (0 = not available, 1 = submission, 2 = assessment, 3 = closed).',
                'optional' => true,
            ),
            'useexamples' => array(
                'type' => PARAM_BOOL,
            ...
            'strategy' => array(
                'type' => PARAM_PLUGIN,
                'description' => 'The type of the current grading strategy used in this peerreview.',
                'optional' => true,
            ),
            'evaluation' => array(
                'type' => PARAM_PLUGIN,
            ...
            'conclusion' => array(
                'type' => PARAM_RAW,
                'description' => 'A text to be displayed at the end of the peerreview.',
                'optional' => true,
                'null' => NULL_ALLOWED,
            ),
            'conclusionformat' => array(
                ...
                'type' => PARAM_INT,
                'default' => FORMAT_MOODLE,
                'description' => 'peerreview conclusion text format.',
            ),
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $values['introfiles'] = external_util::get_area_files($context->id, 'mod_peerreview', 'intro', false, false);

        if (!empty($this->data->instructauthors)) {
            $values['instructauthorsfiles'] = external_util::get_area_files($context->id, 'mod_peerreview', 'instructauthors');
        }

        if (!empty($this->data->instructreviewers)) {
            $values['instructreviewersfiles'] = external_util::get_area_files($context->id, 'mod_peerreview', 'instructreviewers');
        }

        if (!empty($this->data->conclusion)) {
            $values['conclusionfiles'] = external_util::get_area_files($context->id, 'mod_peerreview', 'conclusion');
        }
