...
use mod_peerreview\external\peerreview_summary_exporter;
use mod_peerreview\external\submission_exporter;
use mod_peerreview\external\assessment_exporter;

/**
 * peerreview external functions
 *
 * @package    mod_peerreview
 * @category   external
 * @copyright  2017 Juan Leyva <juan@moodle.com>
 ...
 * @since      Moodle 3.4
 */
class mod_peerreview_external extends external_api {

    /**
     * Describes the parameters for get_peerreviews_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.4
     */
    public static function get_peerreviews_by_courses_parameters() {
    ...
    /**
     * Returns a list of peerreviews in a provided list of courses.
     * If no list is provided all peerreviews that the user can view will be returned.
     *
     * @param array $courseids course ids
     * @return array of warnings and peerreviews
     * @since Moodle 3.4
     */
    public static function get_peerreviews_by_courses($courseids = array()) {
        global $PAGE;

        $warnings = array();
        $returnedpeerreviews = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_peerreviews_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the peerreviews in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $peerreviews = get_all_instances_in_courses("peerreview", $courses);
            foreach ($peerreviews as $peerreview) {

                $context = context_module::instance($peerreview->coursemodule);
                // Remove fields that are not from the peerreview (added by get_all_instances_in_courses).
                unset($peerreview->coursemodule, $peerreview->context, $peerreview->visible, $peerreview->section, $peerreview->groupmode,
                        $peerreview->groupingid);

                $exporter = new peerreview_summary_exporter($peerreview, array('context' => $context));
                $returnedpeerreviews[] = $exporter->export($output);
            }
        }

        $result = array(
            'peerreviews' => $returnedpeerreviews,
            'warnings' => $warnings
        );
        return $result;
    }
    ...
    /**
     * Describes the get_peerreviews_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.4
     */
    public static function get_peerreviews_by_courses_returns() {
        return new external_single_structure(
            array(
                'peerreviews' => new external_multiple_structure(
                    peerreview_summary_exporter::get_read_structure()
                ),
    ...
    /**
     * Utility function for validating a peerreview.
     *
     * @param int $peerreviewid peerreview instance id
     * @return array array containing the peerreview object, course, context and course module objects
     * @since  Moodle 3.4
     */
    protected static function validate_peerreview($peerreviewid) {
        global $DB, $USER;

        // Request and permission validation.
        $peerreview = $DB->get_record('peerreview', array('id' => $peerreviewid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($peerreview, 'peerreview');
        ...
        return array($peerreview, $course, $cm, $context);
    }


    /**
     * Describes the parameters for get_peerreview_access_information.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.4
     */
    public static function get_peerreview_access_information_parameters() {
        return new external_function_parameters (
            array(
                'peerreviewid' => new external_value(PARAM_INT, 'peerreview instance id.')
            )
        );
    }

    /**
     * Return access information for a given peerreview.
     *
     * @param int $peerreviewid peerreview instance id
     ...
     */
    public static function get_peerreview_access_information($peerreviewid) {
        global $USER;

        $params = self::validate_parameters(self::get_peerreview_access_information_parameters(), array('peerreviewid' => $peerreviewid));

        list($peerreview, $course, $cm, $context) = self::validate_peerreview($params['peerreviewid']);

        $result = array();
        // Return all the available capabilities.
        $capabilities = load_capability_def('mod_peerreview');
        foreach ($capabilities as $capname => $capdata) {
            ...
            $field = 'can' . str_replace('mod/peerreview:', '', $capname);
            $result[$field] = has_capability($capname, $context);
        }

        // Now, specific features access information.
        $result['creatingsubmissionallowed'] = $peerreview->creating_submission_allowed($USER->id);
        $result['modifyingsubmissionallowed'] = $peerreview->modifying_submission_allowed($USER->id);
        $result['assessingallowed'] = $peerreview->assessing_allowed($USER->id);
        $result['assessingexamplesallowed'] = $peerreview->assessing_examples_allowed();
        if (is_null($result['assessingexamplesallowed'])) {
            $result['assessingexamplesallowed'] = false;
        }
        $result['examplesassessedbeforesubmission'] = $peerreview->check_examples_assessed_before_submission($USER->id);
        list($result['examplesassessedbeforeassessment'], $code) = $peerreview->check_examples_assessed_before_assessment($USER->id);
    ...
    /**
     * Describes the get_peerreview_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.4
     */
    public static function get_peerreview_access_information_returns() {
     ...
     * @throws  moodle_exception
     */
    public static function get_user_plan($peerreviewid, $userid = 0) {
        global $USER;
        ...
        if (empty($params['userid']) || $params['userid'] == $USER->id) {
            ...
            if (!$peerreview->check_group_membership($user->id)) {
        ...
        $userplan = new peerreview_user_plan($peerreview, $userid);
    ...
    /**
     * Describes the parameters for view_peerreview.
     *
     * @return external_function_parameters
     * @since Moodle 3.4
     */
    public static function view_peerreview_parameters() {
        return new external_function_parameters (
            array(
                'peerreviewid' => new external_value(PARAM_INT, 'peerreview instance id'),
            )
     ...
     * @throws moodle_exception
     */
    public static function view_peerreview($peerreviewid) {

        $params = array('peerreviewid' => $peerreviewid);
        $params = self::validate_parameters(self::view_peerreview_parameters(), $params);
        $warnings = array();
    ...
    /**
     * Describes the view_peerreview return value.
     *
     * @return external_single_structure
     * @since Moodle 3.4
     */
    public static function view_peerreview_returns() {
        return new external_single_structure(
            array(
    ...
    public static function add_submission_parameters() {
        return new external_function_parameters(array(
            'peerreviewid' => new external_value(PARAM_INT, 'peerreview id'),
            ...
            'description' => new external_value(PARAM_RAW, 'Submission text content', VALUE_DEFAULT, ''),
            'descriptionformat' => new external_value(PARAM_INT, 'The format used for the content', VALUE_DEFAULT, FORMAT_MOODLE),
            'descriptioninlineattachmentsid' => new external_value(PARAM_INT, 'The draft file area id for inline attachments in the content',
                VALUE_DEFAULT, 0),
            'descriptionattachmentsid' => new external_value(PARAM_INT, 'The draft file area id for attachments', VALUE_DEFAULT, 0),
        ));
    }

    /**
     * Add a new submission to a given peerreview.
     *
     * @param int $peerreviewid the peerreview id
     ...
     * @throws moodle_exception
     */
    public static function add_submission($peerreviewid, $title, $content = '', $contentformat = FORMAT_MOODLE,
            $inlineattachmentsid = 0, $attachmentsid = 0, $description = '', $descriptionformat = FORMAT_MOODLE,
            $descriptioninlineattachmentsid = 0, $descriptionattachmentsid = 0) {
        global $USER;
        /* EDIT: add description field */
        $params = self::validate_parameters(self::add_submission_parameters(), array(
            ...
            'descriptioninlineattachmentsid' => $descriptioninlineattachmentsid,
            'descriptionattachmentsid' => $descriptionattachmentsid,
        ));
        $warnings = array();

        // Get and validate the peerreview.
        list($peerreview, $course, $cm, $context) = self::validate_peerreview($params['peerreviewid']);
        require_capability('mod/peerreview:submit', $context);

        // Check if we can submit now.
        $canaddsubmission = $peerreview->creating_submission_allowed($USER->id);
        $canaddsubmission = $canaddsubmission && $peerreview->check_examples_assessed_before_submission($USER->id);
        if (!$canaddsubmission) {
        ...
        $submission->groupcoauthor = trim($params['groupcoauthor']);
        ...
        /* EDIT: add description field */
        $submission->description_editor = array(
        ...
        $errors = $peerreview->validate_submission_data((array) $submission);
        ...
        if (!empty($errors)) {
            ...
            $submission->id = $peerreview->edit_submission($submission);
            return array(
                'status' => true,
    ...
    public static function add_submission_returns() {
        return new external_single_structure(array(
            ...
            'submissionid' => new external_value(PARAM_INT, 'New peerreview submission id.', VALUE_OPTIONAL),
            'warnings' => new external_warnings()
        ));
    }
    ...
    public static function update_submission($submissionid, $title, $content = '', $contentformat = FORMAT_MOODLE,
        ...
        // Get and validate the submission and peerreview.
        $submission = $DB->get_record('peerreview_submissions', array('id' => $params['submissionid']), '*', MUST_EXIST);
        list($peerreview, $course, $cm, $context) = self::validate_peerreview($submission->peerreviewid);
        require_capability('mod/peerreview:submit', $context);

        // Check if we can update the submission.
        $canupdatesubmission = $submission->authorid == $USER->id;
        $canupdatesubmission = $canupdatesubmission && $peerreview->modifying_submission_allowed($USER->id);
        $canupdatesubmission = $canupdatesubmission && $peerreview->check_examples_assessed_before_submission($USER->id);
        ...
        $submission->description_editor = array(
            'text' => $params['description'],
            'format' => $params['descriptionformat'],
            'itemid' => $params['descriptioninlineattachmentsid'],
        );
        $submission->attachment_filemanager = $params['attachmentsid'];
    ...
    public static function delete_submission($submissionid) {
        ...
        if (!has_capability('mod/peerreview:deletesubmissions', $context)) {
            require_capability('mod/peerreview:submit', $context);
            // We can delete our own submission, on time and not yet assessed.
            $candeletesubmission = $submission->authorid == $USER->id;
            $candeletesubmission = $candeletesubmission && $peerreview->modifying_submission_allowed($USER->id);
            $candeletesubmission = $candeletesubmission && count($peerreview->get_assessments_of_submission($submission->id)) == 0;
     ...
     * @param  peerreview $peerreview   the peerreview class
     * @param  bool $canviewauthorpublished whether the user has the capability mod/peerreview:viewauthorpublished on
     * @param  bool $canviewauthornames whether the user has the capability mod/peerreview:vviewauthornames on
     * @param  bool $canviewallsubmissions whether the user has the capability mod/peerreview:viewallsubmissions on
     * @return stdClass object with the submission data filtered
     * @since Moodle 3.4
     */
    protected static function prepare_submission_for_external($submission, peerreview $peerreview, $canviewauthorpublished = null,
            $canviewauthornames = null, $canviewallsubmissions = null) {
        global $USER;

        if (is_null($canviewauthorpublished)) {
            $canviewauthorpublished = has_capability('mod/peerreview:viewauthorpublished', $peerreview->context);
        }
        if (is_null($canviewauthornames)) {
            $canviewauthornames = has_capability('mod/peerreview:viewauthornames', $peerreview->context);
        }
        if (is_null($canviewallsubmissions)) {
            $canviewallsubmissions = has_capability('mod/peerreview:viewallsubmissions', $peerreview->context);
        }
        ...
        // WS mod_peerreview_external::get_grades should be used for retrieving grades by students.
        if ($peerreview->phase < peerreview::PHASE_EVALUATION || !$canviewallsubmissions) {
            $properties = submission_exporter::properties_definition();
            foreach ($properties as $attribute => $settings) {
                // Special case, the feedbackauthor (and who did it) should be returned if the peerreview is closed and
                // the user can view it.
                if (($attribute == 'feedbackauthor' || $attribute == 'gradeoverby') &&
                        $peerreview->phase == peerreview::PHASE_CLOSED && $ownsubmission) {
                    continue;
                }
    ...
    /**
     * Retrieves all the peerreview submissions visible by the current user or the one done by the given user
     * (except example submissions).
     *
     * @param int $peerreviewid       the peerreview instance id
     ...
     * @throws moodle_exception
     */
    public static function get_submissions($peerreviewid, $userid = 0, $groupid = 0, $page = 0, $perpage = 0) {
        global $PAGE, $USER;

        $params = array('peerreviewid' => $peerreviewid, 'userid' => $userid, 'groupid' => $groupid,
        ...
        list($submissionsrecords, $totalcount) =
            $peerreview->get_visible_submissions($params['userid'], $groupid, $params['page'], $params['perpage']);

        if ($totalcount) {

            $canviewauthorpublished = has_capability('mod/peerreview:viewauthorpublished', $context);
            $canviewauthornames = has_capability('mod/peerreview:viewauthornames', $context);
            $canviewallsubmissions = has_capability('mod/peerreview:viewallsubmissions', $context);

            $related = array('context' => $context);
            foreach ($submissionsrecords as $submission) {
                $submission = self::prepare_submission_for_external($submission, $peerreview, $canviewauthorpublished,
            ...
            $fs = get_file_storage();
            $files = $fs->get_area_files($context->id, 'mod_peerreview', array('submission_content', 'submission_attachment'));
     ...
     * @param  peerreview   $peerreview     peerreview instance
     * @return void
     * @since  Moodle 3.4
     */
    protected static function validate_submission($submission, peerreview $peerreview) {
        global $USER;

        $peerreviewclosed = $peerreview->phase == peerreview::PHASE_CLOSED;
        $canviewpublished = has_capability('mod/peerreview:viewpublishedsubmissions', $peerreview->context);

        $canview = $submission->authorid == $USER->id;  // I did it.
        $canview = $canview || !empty($peerreview->get_assessment_of_submission_by_user($submission->id, $USER->id));  // I reviewed.
        $canview = $canview || has_capability('mod/peerreview:viewallsubmissions', $peerreview->context); // I can view all.
        $canview = $canview || ($submission->published && $peerreviewclosed && $canviewpublished);    // It has been published.

        if ($canview) {
            // Here we should check if the user share group.
            if ($submission->authorid != $USER->id &&
                    !groups_user_groups_visible($peerreview->course, $submission->authorid, $peerreview->cm)) {
                throw new moodle_exception('notingroup');
            }
        } else {
    ...
    public static function get_submission($submissionid) {
        ...
        self::validate_submission($submission, $peerreview);

        $submission = self::prepare_submission_for_external($submission, $peerreview);
     ...
     * @since  Moodle 3.4
     */
    protected static function check_view_submission_assessments($submission, peerreview $peerreview) {
        global $USER;

        $ownsubmission = $submission->authorid == $USER->id;
        $canview = has_capability('mod/peerreview:viewallassessments', $peerreview->context) ||
            ($ownsubmission && $peerreview->assessments_available());
     ...
     * @since Moodle 3.4
     */
    protected static function prepare_assessment_for_external($assessment, peerreview $peerreview) {
        global $USER;
        ...
        if (is_null($canviewallassessments)) {
            $canviewallassessments = has_capability('mod/peerreview:viewallassessments', $peerreview->context);
        }
        if (is_null($canviewreviewers)) {
            $canviewreviewers = has_capability('mod/peerreview:viewreviewernames', $peerreview->context);
        }
        ...
        if (($peerreview->phase < peerreview::PHASE_EVALUATION || !($isreviewer || $canviewallassessments)) &&
                ($peerreview->phase < peerreview::PHASE_ASSESSMENT || !$canviewallassessments) ) {
    ...
    public static function get_submission_assessments($submissionid) {
        ...
        self::check_view_submission_assessments($submission, $peerreview);
        $assessmentsrecords = $peerreview->get_assessments_of_submission($submission->id);

        $related = array('context' => $context);
        foreach ($assessmentsrecords as $assessment) {
            $assessment = self::prepare_assessment_for_external($assessment, $peerreview);
            if (empty($assessment)) {
                continue;
            }
    ...
    public static function get_assessment($assessmentid) {
        ...
        // Get and validate the assessment, submission and peerreview.
        $assessment = $DB->get_record('peerreview_assessments', array('id' => $params['assessmentid']), '*', MUST_EXIST);
        $submission = $DB->get_record('peerreview_submissions', array('id' => $assessment->submissionid), '*', MUST_EXIST);
        list($peerreview, $course, $cm, $context) = self::validate_peerreview($submission->peerreviewid);
    ...
    public static function get_assessment_form_definition($assessmentid, $mode = 'assessment') {
        ...
        $cansetassessmentweight = has_capability('mod/peerreview:allocate', $context);
        ...
        // Get dimensions info.
        $grader = $peerreview->grading_strategy_instance();
     ...
     * @throws moodle_exception
     */
    public static function get_reviewer_assessments($peerreviewid, $userid = 0) {
        global $USER, $DB, $PAGE;

        $params = self::validate_parameters(
            self::get_reviewer_assessments_parameters(), array('peerreviewid' => $peerreviewid, 'userid' => $userid)
        );
        $warnings = $assessments = array();
        ...
        if (empty($params['userid']) || $params['userid'] == $USER->id) {
            $userid = $USER->id;
            list($assessed, $notice) = $peerreview->check_examples_assessed_before_assessment($userid);
            if (!$assessed) {
                throw new moodle_exception($notice, 'mod_peerreview');
            }
            if ($peerreview->phase < peerreview::PHASE_ASSESSMENT) {    // Can view assessments only in assessment phase onwards.
                throw new moodle_exception('nopermissions', 'error', '', 'view assessments');
            }
        } else {
            require_capability('mod/peerreview:viewallassessments', $context);
            $user = core_user::get_user($params['userid'], '*', MUST_EXIST);
            core_user::require_active_user($user);
            if (!$peerreview->check_group_membership($user->id)) {
                throw new moodle_exception('notingroup');
        ...
        $assessmentsrecords = $peerreview->get_assessments_by_reviewer($userid);
                    ...
                    new external_single_structure(
                        array(
                            'name' => new external_value(PARAM_ALPHANUMEXT,
                                ...
                                anonymousreview (int); the anon key
                                feedbackauthorattachmentsid (int); the draft file area id for the feedbsack attachments'
                            ),
...
public static function update_assessment($assessmentid, $data) {
    ...
    $peerreview->check_edit_assessment($assessment, $submission);

    // Process data.
    $processeddata = new stdClass();
    $processeddata->feedbackauthor_editor = array();

    foreach ($params['data'] as $wsdata) {
        $name = trim($wsdata['name']);
        switch ($name) {
            case 'feedbackauthor':
                $processeddata->feedbackauthor_editor['text'] = $wsdata['value'];
                break;
            case 'feedbackauthorformat':
                $processeddata->feedbackauthor_editor['format'] = clean_param($wsdata['value'], PARAM_FORMAT);
                break;
            case 'feedbackauthorinlineattachmentsid':
                $processeddata->feedbackauthor_editor['itemid'] = clean_param($wsdata['value'], PARAM_INT);
                break;
            case 'feedbackauthorattachmentsid':
                $processeddata->feedbackauthorattachment_filemanager = clean_param($wsdata['value'], PARAM_INT);
                break;
            case 'anonymousreview':
                $processeddata->anonymousreview = clean_param($wsdata['value'], PARAM_INT);
                break;
            default:
                $processeddata->{$wsdata['name']} = $wsdata['value']; // Validation will be done in the form->validation.
        }
    }
    ...
    // Retrieve the data from the strategy plugin.
    $strategy = $peerreview->grading_strategy_instance();
    $mform = $strategy->get_assessment_form(null, 'assessment', $assessment, true, array(
        'editableweight' => $cansetassessmentweight,
        'pending' => !empty($pending)
    ));

    $errors = $mform->validation((array)$processeddata, array());
    // We can get several errors, return them in warnings.
    if (!empty($errors)) {
        $status = false;
        $rawgrade = null;
        ...
        $rawgrade = $peerreview->edit_assessment($assessment, $submission, $processeddata, $strategy);
        $status = true;
    }
    ...
    /**
     * Returns the grades information for the given peerreview and user.
     *
     * @param int $peerreviewid peerreview instance id
     * @param int $userid user id
     * @return array of warnings and the user plan
     ...
     * @throws  moodle_exception
     */
    public static function get_grades($peerreviewid, $userid = 0) {
        global $USER;
        ...
        $finalgrades = $peerreview->get_gradebook_grades($userid);
    ...
    public static function evaluate_assessment($assessmentid, $feedbacktext = '', $feedbackformat = FORMAT_MOODLE, $weight = 1,
        ...
        $peerreview->check_view_assessment($assessment, $submission);
        $cansetassessmentweight = has_capability('mod/peerreview:allocate', $context);
        $canoverridegrades      = has_capability('mod/peerreview:overridegrades', $context);
        ...
        $feedbackform = $peerreview->get_feedbackreviewer_form(null, $assessment, $options);

        $errors = $feedbackform->validation((array) $data, array());
        // Extra checks for the new grade and weight.
        $possibleweights = peerreview::available_assessment_weights_list();
        ...
        if (is_numeric($data->gradinggradeover) &&
                ($data->gradinggradeover < 0 || $data->gradinggradeover > $peerreview->gradinggrade)) {
     ...
     * @throws moodle_exception
     */
    public static function get_grades_report($peerreviewid, $groupid = 0, $sortby = 'lastname', $sortdirection = 'ASC',
            $page = 0, $perpage = 0) {
        global $USER;

        $params = array('peerreviewid' => $peerreviewid, 'groupid' => $groupid, 'sortby' => $sortby, 'sortdirection' => $sortdirection,
            'page' => $page, 'perpage' => $perpage);
        ...
        if ($peerreview->phase >= peerreview::PHASE_SUBMISSION) {
            $showauthornames = has_capability('mod/peerreview:viewauthornames', $context);
            $showreviewernames = has_capability('mod/peerreview:viewreviewernames', $context);

            if ($peerreview->phase >= peerreview::PHASE_EVALUATION) {
            ...
            $data = $peerreview->prepare_grading_report_data($USER->id, $groupid, $params['page'], $params['perpage'],
            ...
            if (!empty($data)) {
                ...
                $report = new peerreview_grading_report($data, $reportopts);
                return array(
        ...
        throw new moodle_exception('nothingfound', 'peerreview');
    }
    ...
    public static function evaluate_submission($submissionid, $feedbacktext = '', $feedbackformat = FORMAT_MOODLE, $published = 1,
        ...
        // Get and validate the submission, submission and peerreview.
        $submission = $DB->get_record('peerreview_submissions', array('id' => $params['submissionid']), '*', MUST_EXIST);
        list($peerreview, $course, $cm, $context) = self::validate_peerreview($submission->peerreviewid);

        // Check we can evaluate the submission.
        self::validate_submission($submission, $peerreview);
        $canpublish  = has_capability('mod/peerreview:publishsubmissions', $context);
        $canoverride = ($peerreview->phase == peerreview::PHASE_EVALUATION &&
            has_capability('mod/peerreview:overridegrades', $context));
        ...
        $feedbackform = $peerreview->get_feedbackauthor_form(null, $submission, $options);

        $errors = $feedbackform->validation((array) $data, array());
        // Extra checks for the new grade (if set).
        if (is_numeric($data->gradeover) && $data->gradeover > $peerreview->grade) {
