...
/**
 * Defines {@link \mod_peerreview\privacy\provider} class.
 *
...
/**
 * Privacy API implementation for the peerreview activity module.
 *
 * @copyright 2018 David Mudrák <david@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements
    ...
    /**
     * Describe all the places where the peerreview module stores some personal data.
     *
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('peerreview_submissions', [
            'peerreviewid' => 'privacy:metadata:peerreviewid',
        ...
        ], 'privacy:metadata:peerreviewsubmissions');

        $collection->add_database_table('peerreview_assessments', [
        ...
        ], 'privacy:metadata:peerreviewassessments');

        $collection->add_database_table('peerreview_grades', [
        ...
        ], 'privacy:metadata:peerreviewgrades');

        $collection->add_database_table('peerreview_aggregations', [
            'peerreviewid' => 'privacy:metadata:peerreviewid',
            'userid' => 'privacy:metadata:userid',
            'gradinggrade' => 'privacy:metadata:aggregatedgradinggrade',
            'timegraded' => 'privacy:metadata:timeaggregated',
        ], 'privacy:metadata:peerreviewaggregations');
        ...
        foreach ($userprefs as $userpref) {
            if ($userpref === 'peerreview_perpage') {
                $collection->add_user_preference('peerreview_perpage', 'privacy:metadata:preference:perpage');
            } else {
                $summary = str_replace('peerreview-', '', $userpref);
     ...
     *
     * User has personal data in the peerreview if any of the following cases happens:
     *
     * - the user has submitted in the peerreview
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = "SELECT ctx.id
             ...
             LEFT JOIN {peerreview_submissions} ws ON ws.peerreviewid = w.id
             LEFT JOIN {peerreview_assessments} wa ON wa.submissionid = ws.id AND (
                    wa.reviewerid = :wareviewerid
                        OR
                    wa.gradinggradeoverby = :wagradinggradeoverby
                )
             LEFT JOIN {peerreview_aggregations} wr ON wr.peerreviewid = w.id AND wr.userid = :wruserid
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $sql = "SELECT ws.authorid, ws.gradeoverby, wa.reviewerid, wa.gradinggradeoverby, wr.userid
                  ...
                  JOIN {modules} m ON cm.module = m.id AND m.name = :module
                  JOIN {peerreview} w ON cm.instance = w.id
                  JOIN {peerreview_submissions} ws ON ws.peerreviewid = w.id
             LEFT JOIN {peerreview_assessments} wa ON wa.submissionid = ws.id
             LEFT JOIN {peerreview_aggregations} wr ON wr.peerreviewid = w.id
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        // Export general information about all peerreviews.
        foreach ($contextlist->get_contexts() as $context) {
            if ($context->contextlevel != CONTEXT_MODULE) {
                continue;
            }
            $data = helper::get_context_data($context, $user);
            static::append_extra_peerreview_data($context, $user, $data, []);
    ...
    public static function export_user_preferences(int $userid) {
        ...
        foreach ($userprefs as $userpref) {
            ...
            if ($userprefval !== null) {
                $langid = str_replace('peerreview-', '', $userpref);
                $description = get_string("privacy:metadata:preference:$langid", 'mod_peerreview');
                if ($userpref === 'peerreview_perpage') {
                    writer::export_user_preference('mod_peerreview', $userpref, $userprefval,
                            get_string('privacy:metadata:preference:perpage', 'mod_peerreview'));
                } else {
                    writer::export_user_preference('mod_peerreview', $userpref,
    ...
    /**
     * Append additional relevant data into the base data about the peerreview instance.
     *
     ...
     * was the phase of the peerreview when it was exported.
     *
     * @param \context $context peerreview module content.
     * @param stdClass $user User for which we are exporting data.
     * @param stdClass $data Base data about the peerreview instance to append to.
     * @param array $subcontext Subcontext path items to eventually write files into.
     */
    protected static function append_extra_peerreview_data(\context $context, \stdClass $user, \stdClass $data, array $subcontext) {
        global $DB;
        ...
        if ($record->phase >= \peerreview::PHASE_SUBMISSION) {
            $data->instructauthors = $writer->rewrite_pluginfile_urls($subcontext, 'mod_peerreview', 'instructauthors', 0,
                $record->instructauthors);
            $data->instructauthorsformat = $record->instructauthorsformat;
        }

        if ($record->phase >= \peerreview::PHASE_ASSESSMENT) {
            $data->instructreviewers = $writer->rewrite_pluginfile_urls($subcontext, 'mod_peerreview', 'instructreviewers', 0,
                $record->instructreviewers);
            $data->instructreviewersformat = $record->instructreviewersformat;
        }

        if ($record->phase >= \peerreview::PHASE_CLOSED) {
            $data->conclusion = $writer->rewrite_pluginfile_urls($subcontext, 'mod_peerreview', 'conclusion', 0, $record->conclusion);
            $data->conclusionformat = $record->conclusionformat;
        }

        $data->strategy = \peerreview::available_strategies_list()[$record->strategy];
        $data->evaluation = \peerreview::available_evaluators_list()[$record->evaluation];
        ...
        switch ($record->phase) {
            case \peerreview::PHASE_SETUP:
                $data->phase = get_string('phasesetup', 'mod_peerreview');
                break;
            case \peerreview::PHASE_SUBMISSION:
                $data->phase = get_string('phasesubmission', 'mod_peerreview');
                break;
            case \peerreview::PHASE_ASSESSMENT:
                $data->phase = get_string('phaseassessment', 'mod_peerreview');
                break;
            case \peerreview::PHASE_EVALUATION:
                $data->phase = get_string('phaseevaluation', 'mod_peerreview');
                break;
            case \peerreview::PHASE_CLOSED:
                $data->phase = get_string('phaseclosed', 'mod_peerreview');
                break;
        }

        $writer->export_area_files($subcontext, 'mod_peerreview', 'instructauthors', 0);
        $writer->export_area_files($subcontext, 'mod_peerreview', 'instructreviewers', 0);
        $writer->export_area_files($subcontext, 'mod_peerreview', 'conclusion', 0);
    }
    ...
    protected static function export_submissions(approved_contextlist $contextlist) {
        ...
        $sql = "SELECT ws.id, ws.authorid, ws.example, ws.timecreated, ws.timemodified, ws.title,
                  ...
                  JOIN {context} ctx ON ctx.contextlevel = :contextlevel AND ctx.instanceid = cm.id
                  JOIN {peerreview} w ON cm.instance = w.id
                  JOIN {peerreview_submissions} ws ON ws.peerreviewid = w.id AND ws.authorid = :authorid
        ...
        foreach ($rs as $record) {
            ...
            if ($record->example) {
                $subcontext = [get_string('examplesubmissions', 'mod_peerreview'), $record->id];
                $mysubmission = null;
            } else {
                $subcontext = [get_string('mysubmission', 'mod_peerreview')];
                $mysubmission = $record;
            }
            ...
            $data = (object) [
                ...
                'title' => $record->title,
                'content' => $writer->rewrite_pluginfile_urls($subcontext, 'mod_peerreview',
            ...
            $writer->export_area_files($subcontext, 'mod_peerreview', 'submission_content', $record->id);
            $writer->export_area_files($subcontext, 'mod_peerreview', 'submission_attachment', $record->id);

            // Export peer-assessments of my submission if the peerreview was closed. We do not export received
            // assessments from peers before they were actually effective. Before the peerreview is closed, grades are not
            ...
            if ($mysubmission && $phase == \peerreview::PHASE_CLOSED) {
                $assessments = $DB->get_records('peerreview_assessments', ['submissionid' => $mysubmission->id], '',
                ...
                foreach ($assessments as $assessment) {
                    ...
                    $assessment->feedbackauthor = $writer->rewrite_pluginfile_urls($subcontext,
                        'mod_peerreview', 'overallfeedback_content', $assid, $assessment->feedbackauthor);

                    $assessmentsubcontext = array_merge($subcontext, [get_string('assessments', 'mod_peerreview'), $assid]);

                    unset($assessment->id);
                    unset($assessment->reviewerid);

                    $writer->export_data($assessmentsubcontext, $assessment);
                    $writer->export_area_files($assessmentsubcontext, 'mod_peerreview', 'overallfeedback_content', $assid);
                    $writer->export_area_files($assessmentsubcontext, 'mod_peerreview', 'overallfeedback_attachment', $assid);
    ...
    protected static function export_assessments(approved_contextlist $contextlist) {
        ...
        $sql = "SELECT ws.authorid, ws.example, ws.timecreated, ws.timemodified, ws.title, ws.content, ws.contentformat,
                  ...
                  JOIN {peerreview} w ON cm.instance = w.id
                  JOIN {peerreview_submissions} ws ON ws.peerreviewid = w.id
                  JOIN {peerreview_assessments} wa ON wa.submissionid = ws.id AND wa.reviewerid = :reviewerid
                 WHERE ctx.id {$contextsql}";
        ...
        foreach ($rs as $record) {
            ...
            $context = \context_module::instance($record->cmid);
            $writer = \core_privacy\local\request\writer::with_context($context);
            $subcontext = [get_string('myassessments', 'mod_peerreview'), $record->id];
            ...
            $data = (object) [
                ...
                'feedbackauthor' => $writer->rewrite_pluginfile_urls($subcontext, 'mod_peerreview',
            ...
            $writer->export_area_files($subcontext, 'mod_peerreview', 'overallfeedback_content', $record->id);
            $writer->export_area_files($subcontext, 'mod_peerreview', 'overallfeedback_attachment', $record->id);
            $writer->export_area_files($subcontext, 'mod_peerreview', 'submission_content', $record->submissionid);
            $writer->export_area_files($subcontext, 'mod_peerreview', 'submission_attachment', $record->submissionid);
     ...
     * @param context $context peerreview activity content
    ...
    protected static function export_assessment_forms(\stdClass $user, \context $context, array $subcontext, int $assessmentid) {
        ...
        foreach (\peerreview::available_strategies_list() as $strategy => $title) {
            $providername = '\peerreviewform_'.$strategy.'\privacy\provider';

            if (is_subclass_of($providername, '\mod_peerreview\privacy\peerreviewform_provider')) {
                component_class_callback($providername, 'export_assessment_form',
                    [
                        $user,
                        $context,
                        array_merge($subcontext, [get_string('assessmentform', 'mod_peerreview'), $title]),
                        $assessmentid,
                    ]
                );

            } else {
                debugging('Missing class '.$providername.' implementing peerreviewform_provider interface', DEBUG_DEVELOPER);
            }
        }
    }
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $cm = get_coursemodule_from_id('peerreview', $context->instanceid, 0, false, IGNORE_MISSING);
        ...
        $peerreview = $DB->get_record('peerreview', ['id' => $cm->instance], 'id, course', MUST_EXIST);

        $submissions = $DB->get_records('peerreview_submissions', ['peerreviewid' => $peerreview->id], '', 'id');
        $assessments = $DB->get_records_list('peerreview_assessments', 'submissionid', array_keys($submissions), '', 'id');

        $DB->delete_records('peerreview_aggregations', ['peerreviewid' => $peerreview->id]);
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'mod_peerreview', 'submission_content');
        $fs->delete_area_files($context->id, 'mod_peerreview', 'submission_attachment');
        $fs->delete_area_files($context->id, 'mod_peerreview', 'overallfeedback_content');
        $fs->delete_area_files($context->id, 'mod_peerreview', 'overallfeedback_attachment');

        grade_update('mod/peerreview', $peerreview->course, 'mod', 'peerreview', $peerreview->id, 0, null, ['reset' => true]);
        grade_update('mod/peerreview', $peerreview->course, 'mod', 'peerreview', $peerreview->id, 1, null, ['reset' => true]);
     ...
     *
     * Removing assessments of submissions from the peerreview is not trivial. Removing one user's data can easily affect
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        if ($submissionids) {
            ...
            $DB->set_field_select('peerreview_submissions', 'title', get_string('privacy:request:delete:title',
                'mod_peerreview'), "id $submissionidsql", $submissionidparams);
            $DB->set_field_select('peerreview_submissions', 'content', get_string('privacy:request:delete:content',
                'mod_peerreview'), "id $submissionidsql", $submissionidparams);
            $DB->set_field_select('peerreview_submissions', 'feedbackauthor', get_string('privacy:request:delete:content',
                'mod_peerreview'), "id $submissionidsql", $submissionidparams);

            foreach ($contextlist->get_contextids() as $contextid) {
                $fs->delete_area_files_select($contextid, 'mod_peerreview', 'submission_content',
                    $submissionidsql, $submissionidparams);
                $fs->delete_area_files_select($contextid, 'mod_peerreview', 'submission_attachment',
                    $submissionidsql, $submissionidparams);
            }
        }
        ...
        $sql = "SELECT wa.id AS assessmentid
                  ...
                  JOIN {peerreview} w ON cm.instance = w.id
                  JOIN {peerreview_submissions} ws ON ws.peerreviewid = w.id AND ws.authorid = :authorid
                  JOIN {peerreview_assessments} wa ON wa.submissionid = ws.id
                 WHERE ctx.id {$contextsql}";
        ...
        if ($assessmentids) {
            ...
            $DB->set_field_select('peerreview_assessments', 'feedbackauthor', get_string('privacy:request:delete:content',
                'mod_peerreview'), "id $assessmentidsql", $assessmentidparams);

            foreach ($contextlist->get_contextids() as $contextid) {
                $fs->delete_area_files_select($contextid, 'mod_peerreview', 'overallfeedback_content',
                    $assessmentidsql, $assessmentidparams);
                $fs->delete_area_files_select($contextid, 'mod_peerreview', 'overallfeedback_attachment',
                    $assessmentidsql, $assessmentidparams);
            }
        }
        ...
        if ($assessmentids) {
            ...
            $DB->set_field_select('peerreview_assessments', 'feedbackreviewer', get_string('privacy:request:delete:content',
                'mod_peerreview'), "id $assessmentidsql", $assessmentidparams);
        }
    ...
    /**
     * Delete personal data for multiple users within a single peerreview context.
     *
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        $sql = "SELECT ws.id AS submissionid
                  FROM {peerreview} w
                  JOIN {peerreview_submissions} ws ON ws.peerreviewid = w.id
                 WHERE w.id = :peerreviewid AND ws.authorid $usersql";
        ...
        if ($submissionids) {
            ...
            $fs->delete_area_files_select($context->id, 'mod_peerreview', 'submission_content',
                $submissionidsql, $submissionidparams);
            $fs->delete_area_files_select($context->id, 'mod_peerreview', 'submission_attachment',
                $submissionidsql, $submissionidparams);
        }
        ...
        if ($assessmentids) {
            ...
            $fs->delete_area_files_select($context->id, 'mod_peerreview', 'overallfeedback_content',
                $assessmentidsql, $assessmentidparams);
            $fs->delete_area_files_select($context->id, 'mod_peerreview', 'overallfeedback_attachment',
                $assessmentidsql, $assessmentidparams);
        }
        ...
        $sql = "SELECT wa.id AS assessmentid
                  ...
                  JOIN {peerreview_submissions} ws ON ws.peerreviewid = w.id
                  JOIN {peerreview_assessments} wa ON wa.submissionid = ws.id
                 WHERE w.id = :peerreviewid AND wa.reviewerid $usersql";
    ...
    protected static function get_user_prefs(): array {
        return [
            ...
            'peerreview-viewlet-allsubmissions-collapsed',
