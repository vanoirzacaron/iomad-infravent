...
/**
 * Library of internal classes and functions for module peerreview
 *
 * All the peerreview specific functions, needed to implement the module
 * logic, should go to here. Instead of having bunch of function named
 * peerreview_something() taking the peerreview instance as the first
 * parameter, we use a class peerreview that provides all methods.
 *
...
/**
 * Full-featured peerreview API
 *
 * This wraps the peerreview database record with a set of methods that are called
 * from the module itself. The class should be initialized right after you get
 * $peerreview, $cm and $course records at the begining of the script.
 */
class peerreview
{
    ...
    /** the internal code of the peerreview phases as are stored in the database */
    ...
    /** @var stdclass peerreview record from database */
    public $dbrecord;
    ...
    /** @var int peerreview instance identifier */
    public $id;

    /** @var string peerreview activity name */
    public $name;
    ...
    /** @var int current phase of peerreview, for example {@link peerreview::PHASE_SETUP} */
    public $phase;
    ...
    /** @var string type of the current grading strategy used in this peerreview, for example 'accumulative' */
    public $strategy;
    ...
    /** @var int mode of example submissions support, for example {@link peerreview::EXAMPLES_VOLUNTARY} */
    public $examplesmode;
    ...
    /**
     * @var peerreview_strategy grading strategy instance
     * Do not use directly, get the instance using {@link peerreview::grading_strategy_instance()}
     */
    protected $strategyinstance = null;

    /**
     * @var peerreview_evaluation grading evaluation instance
     * Do not use directly, get the instance using {@link peerreview::grading_evaluation_instance()}
     */
    ...
    /**
     * Initializes the peerreview API instance using the data from DB
     *
    ...
    {
        $installed = core_component::get_plugin_list('peerreviewallocation');
        $forms = array();
        foreach ($installed as $allocation => $allocationpath) {
            if (file_exists($allocationpath . '/lib.php')) {
                $forms[$allocation] = get_string('pluginname', 'peerreviewallocation_' . $allocation);
            }
        }
    ...
    {
        return array(
            'subdirs' => 1, 'maxbytes' => 0, 'maxfiles' => -1,
    ...
    {
        $options = array();
        $options[self::EXAMPLES_VOLUNTARY]         = get_string('examplesvoluntary', 'peerreview');
        $options[self::EXAMPLES_BEFORE_SUBMISSION] = get_string('examplesbeforesubmission', 'peerreview');
        $options[self::EXAMPLES_BEFORE_ASSESSMENT] = get_string('examplesbeforeassessment', 'peerreview');
        return $options;
    }
    ...
    {
        $installed = core_component::get_plugin_list('peerreviewform');
        $forms = array();
        foreach ($installed as $strategy => $strategypath) {
            if (file_exists($strategypath . '/lib.php')) {
                $forms[$strategy] = get_string('pluginname', 'peerreviewform_' . $strategy);
            }
        }
        return $forms;
    }
    ...
    {
        $evals = array();
        foreach (core_component::get_plugin_list_with_file('peerrevieweval', 'lib.php', false) as $eval => $evalpath) {
            $evals[$eval] = get_string('pluginname', 'peerrevieweval_' . $eval);
        }
        return $evals;
    }
     ...
     * peerreview 1.x and I just decided to use the same number as in the maximum weight of
    ...
    {
        $formats = array(
            'date', 'datefullshort', 'dateshort', 'datetime',
        ...
        if ($day == $today) {
            $a->distanceday = get_string('daystoday', 'peerreview');
        } elseif ($day == $yesterday) {
            $a->distanceday = get_string('daysyesterday', 'peerreview');
        } elseif ($day < $today) {
            $a->distanceday = get_string('daysago', 'peerreview', $distance);
        } elseif ($day == $tomorrow) {
            $a->distanceday = get_string('daystomorrow', 'peerreview');
        } elseif ($day > $today) {
            $a->distanceday = get_string('daysleft', 'peerreview', $distance);
        }
        return $a;
    }
    ...
    public static function normalize_file_extensions($extensions)
    {
        ...
        debugging('The method peerreview::normalize_file_extensions() is deprecated.
    ...
    public static function clean_file_extensions($extensions)
    {
        ...
        debugging('The method peerreview::clean_file_extensions() is deprecated.
            Please use the methods provided by the \core_form\filetypes_util class.', DEBUG_DEVELOPER);
    ...
    public static function invalid_file_extensions($extensions, $allowlist)
    {
        ...
        debugging('The method peerreview::invalid_file_extensions() is deprecated.
            Please use the methods provided by the \core_form\filetypes_util class.', DEBUG_DEVELOPER);
    ...
    /**
     * Is the file have allowed to be uploaded to the peerreview?
     *
    ...
    public static function is_allowed_file_type($filename, $allowlist)
    {
        ...
        debugging('The method peerreview::is_allowed_file_type() is deprecated.
            Please use the methods provided by the \core_form\filetypes_util class.', DEBUG_DEVELOPER);
    ...
    /**
     * Fetches all enrolled users with the capability mod/peerreview:submit in the current peerreview
     *
    ...
    public function get_potential_authors($musthavesubmission = true, $groupid = 0, $limitfrom = 0, $limitnum = 0)
    {
        ...
        list($sql, $params) = $this->get_users_with_capability_sql('mod/peerreview:submit', $musthavesubmission, $groupid);
    ...
    /**
     * Fetches all enrolled users with the capability mod/peerreview:peerassess in the current peerreview
     *
     * The returned objects contain properties required by user_picture and are ordered by lastname, firstname.
     * Only users with the active enrolment are returned.
     *
    ...
    public function get_potential_reviewers($musthavesubmission = false, $groupid = 0, $limitfrom = 0, $limitnum = 0)
    {
        ...
        list($sql, $params) = $this->get_users_with_capability_sql('mod/peerreview:peerassess', $musthavesubmission, $groupid);
    ...
    /**
     * Fetches all enrolled users that are authors or reviewers (or both) in the current peerreview
     *
     * The returned objects contain properties required by user_picture and are ordered by lastname, firstname.
     * Only users with the active enrolment are returned.
     *
    ...
    /**
     * Checks if the given user is an actively enrolled participant in the peerreview
     *
    ...
    public function get_grouped($users)
    {
        ...
        if ($this->cm->groupingid) {
            // Group peerreview set to specified grouping - only consider groups
        ...
        $gmemberships = groups_get_all_groups(
            $this->cm->course,
            array_keys($users),
            $groupingid,
    ...
    /**
     * Returns the list of all allocations (i.e. assigned assessments) in the peerreview
     *
    ...
    public function get_allocations()
    {
        ...
        $sql = 'SELECT a.id, a.submissionid, a.reviewerid, s.authorid
            ...
            INNER JOIN {peerreview_submissions} s ON (a.submissionid = s.id)
                 WHERE s.example = 0 AND s.peerreviewid = :peerreviewid';
        $params = array('peerreviewid' => $this->id);
    ...
    public function count_submissions($authorid = 'all', $groupid = 0)
    {
        ...
        $sql .= " WHERE s.example = 0 AND s.peerreviewid = :peerreviewid";
    ...
    /**
     * Returns submissions from this peerreview
     *
     * Fetches data from {peerreview_submissions} and adds some useful information from other
    ...
    public function get_submissions($authorid = 'all', $groupid = 0, $limitfrom = 0, $limitnum = 0)
    {
        ...
        $params            = array('peerreviewid' => $this->id);
        $sql = "SELECT s.id, s.peerreviewid, s.example, s.authorid, s.timecreated, s.timemodified,
                       s.title, s.grade, s.groupcoauthor, s.gradeover, s.gradeoverby, s.published,
    ...
    /**
     * Returns submissions from this peerreview that are viewable by the current user (except example submissions).
     *
    ...
    public function get_visible_submissions($authorid = 0, $groupid = 0, $limitfrom = 0, $limitnum = 0)
    {
        ...
        $from = " FROM {peerreview_submissions} s";
        $params = array('peerreviewid' => $this->id);
        ...
        $where = " WHERE s.peerreviewid = :peerreviewid AND s.example = 0";

        if (!has_capability('mod/peerreview:viewallsubmissions', $this->context)) {
            // Check published submissions.
            $peerreviewclosed = $this->phase == self::PHASE_CLOSED;
            $canviewpublished = has_capability('mod/peerreview:viewpublishedsubmissions', $this->context);
            if ($peerreviewclosed && $canviewpublished) {
    ...
    public function get_submission_by_id($id)
    {
        ...
        // we intentionally check the peerreviewid here, too, so the peerreview can't touch submissions
        ...
        $sql = "SELECT s.*, $authorfields, $gradeoverbyfields
            ...
            INNER JOIN {user} u ON (s.authorid = u.id)
             LEFT JOIN {user} g ON (s.gradeoverby = g.id)
                 WHERE s.example = 0 AND s.peerreviewid = :peerreviewid AND s.id = :id";
        $params = array('peerreviewid' => $this->id, 'id' => $id);
    ...
    public function get_submission_by_author($authorid)
    {
        ...
        $sql = "SELECT s.*, $authorfields, $gradeoverbyfields
            ...
            INNER JOIN {user} u ON (s.authorid = u.id)
             LEFT JOIN {user} g ON (s.gradeoverby = g.id)
                 WHERE s.example = 0 AND s.peerreviewid = :peerreviewid AND s.authorid = :authorid";
        $params = array('peerreviewid' => $this->id, 'authorid' => $authorid);
        $subsbyuser = $DB->get_record_sql($sql, $params);
        if(!$subsbyuser) {

                $sql2 = "SELECT s.*
               FROM {peerreview_submissions} s
              WHERE s.example = 0 
                AND s.peerreviewid = :peerreviewid 
                AND s.groupcoauthor LIKE :email";
    $params2 = array('peerreviewid' => $this->id, 'email' => '%'.$USER->email.'%');
    $getsubmissionparams = $DB->get_records_sql($sql2, $params2);
    $originalauthorid = null;
    foreach($getsubmissionparams as $asubmission) {
        $originalauthorid = $asubmission->authorid;
    }
        ...
        $sql = "SELECT s.*, $authorfields, $gradeoverbyfields
            ...
            INNER JOIN {user} u ON (s.authorid = u.id)
             LEFT JOIN {user} g ON (s.gradeoverby = g.id)
                 WHERE s.example = 0 AND s.peerreviewid = :peerreviewid AND s.authorid = :authorid";
        $params = array('peerreviewid' => $this->id, 'authorid' => $originalauthorid);
        $subsbyuser = $DB->get_record_sql($sql, $params);
    ...
    public function get_published_submissions($orderby = 'finalgrade DESC')
    {
        ...
        $sql = "SELECT s.id, s.authorid, s.timecreated, s.timemodified,
                  ...
                  FROM {peerreview_submissions} s
            INNER JOIN {user} u ON (s.authorid = u.id)
                 WHERE s.example = 0 AND s.peerreviewid = :peerreviewid AND s.published = 1
              ORDER BY $orderby";
        $params = array('peerreviewid' => $this->id);
        return $DB->get_records_sql($sql, $params);
    }
    ...
    public function get_example_by_id($id)
    {
        ...
        return $DB->get_record(
            ...
            array('id' => $id, 'peerreviewid' => $this->id, 'example' => 1),
            '*',
            MUST_EXIST
        );
    }
    ...
    /**
     * Returns the list of example submissions in this peerreview with reference assessments attached
     *
     * @return array of objects or an empty array
     * @see peerreview::prepare_example_summary()
     */
    public function get_examples_for_manager()
    {
        ...
        $sql = 'SELECT s.id, s.title,
                  ...
                  FROM {peerreview_submissions} s
             LEFT JOIN {peerreview_assessments} a ON (a.submissionid = s.id AND a.weight = 1)
                 WHERE s.example = 1 AND s.peerreviewid = :peerreviewid
              ORDER BY s.title';
        return $DB->get_records_sql($sql, array('peerreviewid' => $this->id));
    }

    /**
     * Returns the list of all example submissions in this peerreview with the information of assessments done by the given user
     *
    ...
    public function get_examples_for_reviewer($reviewerid)
    {
        ...
        $sql = 'SELECT s.id, s.title,
                       a.id AS assessmentid, a.grade, a.gradinggrade
                  FROM {peerreview_submissions} s
             LEFT JOIN {peerreview_assessments} a ON (a.submissionid = s.id AND a.reviewerid = :reviewerid AND a.weight = 0)
                 WHERE s.example = 1 AND s.peerreviewid = :peerreviewid
              ORDER BY s.title';
        return $DB->get_records_sql($sql, array('peerreviewid' => $this->id, 'reviewerid' => $reviewerid));
    }

    /**
     * Prepares renderable submission component
     *
     * @param stdClass $record required by {@see peerreview_submission}
     * @param bool $showauthor show the author-related information
     * @return peerreview_submission
     */
    public function prepare_submission(stdClass $record, $showauthor = false)
    {

        $submission         = new peerreview_submission($this, $record, $showauthor);
     ...
     *
     * @param stdClass $record required by {@see peerreview_submission_summary}
     * @param bool $showauthor show the author-related information
     * @return peerreview_submission_summary
     */
    public function prepare_submission_summary(stdClass $record, $showauthor = false)
    {

        $summary        = new peerreview_submission_summary($this, $record, $showauthor);
     ...
     *
     * @param stdClass $record required by {@see peerreview_example_submission}
     * @return peerreview_example_submission
     */
    public function prepare_example_submission(stdClass $record)
    {

        $example = new peerreview_example_submission($this, $record);
     ...
     *
     * @param stdClass $example as returned by {@link peerreview::get_examples_for_manager()} or {@link peerreview::get_examples_for_reviewer()}
     * @return peerreview_example_submission_summary to be rendered
     */
    public function prepare_example_summary(stdClass $example)
    {

        $summary = new peerreview_example_submission_summary($this, $example);

        if (is_null($example->grade)) {
            $summary->status = 'notgraded';
            $summary->assesslabel = get_string('assess', 'peerreview');
        } else {
            $summary->status = 'graded';
            $summary->assesslabel = get_string('reassess', 'peerreview');
        }
     ...
     * @param peerreview_assessment_form|null $form as returned by {@link peerreview_strategy::get_assessment_form()}
     * @param array $options
     * @return peerreview_assessment
     */
    public function prepare_assessment(stdClass $record, $form, array $options = array())
    {

        $assessment             = new peerreview_assessment($this, $record, $options);
        $assessment->url        = $this->assess_url($record->id);
        $assessment->maxgrade   = $this->real_grade(100);

        if (!empty($options['showform']) and !($form instanceof peerreview_assessment_form)) {
            debugging('Not a valid instance of peerreview_assessment_form supplied', DEBUG_DEVELOPER);
        }
     ...
     * @param peerreview_assessment_form|null $form as returned by {@link peerreview_strategy::get_assessment_form()}
     * @param array $options
     * @return peerreview_example_assessment
     */
    public function prepare_example_assessment(stdClass $record, $form = null, array $options = array())
    {

        $assessment             = new peerreview_example_assessment($this, $record, $options);
     ...
     * @param peerreview_assessment_form|null $form as returned by {@link peerreview_strategy::get_assessment_form()}
     * @param array $options
     * @return peerreview_example_reference_assessment
     */
    public function prepare_example_reference_assessment(stdClass $record, $form = null, array $options = array())
    {

        $assessment             = new peerreview_example_reference_assessment($this, $record, $options);
        $assessment->maxgrade   = $this->real_grade(100);
    ...
    public function delete_submission(stdclass $submission)
    {
        ...
        $assessments = $DB->get_records('peerreview_assessments', array('submissionid' => $submission->id), '', 'id');
        $this->delete_assessment(array_keys($assessments));

        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'mod_peerreview', 'submission_content', $submission->id);
        $fs->delete_area_files($this->context->id, 'mod_peerreview', 'description_content', $submission->id);
        $fs->delete_area_files($this->context->id, 'mod_peerreview', 'submission_attachment', $submission->id);

        $DB->delete_records('peerreview_submissions', array('id' => $submission->id));
        ...
        $event = \mod_peerreview\event\submission_deleted::create($params);
        $event->add_record_snapshot('peerreview', $this->dbrecord);
        $event->trigger();
    }

    /**
     * Returns the list of all assessments in the peerreview with some data added
     *
     * Fetches data from {peerreview_assessments} and adds some useful information from other
        ...
        $sql = "SELECT a.id, a.submissionid, a.reviewerid, a.timecreated, a.timemodified,
            ...
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.peerreviewid = :peerreviewid AND s.example = 0
              ORDER BY $sort";
        $params['peerreviewid'] = $this->id;
    ...
    public function get_assessment_by_id($id)
    {
        ...
        $sql = "SELECT a.*, s.title, $reviewerfields, $authorfields, $overbyfields
            ...
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE a.id = :id AND s.peerreviewid = :peerreviewid";
        $params = array('id' => $id, 'peerreviewid' => $this->id);
    ...
    public function get_assessment_of_submission_by_user($submissionid, $reviewerid)
    {
        ...
        $sql = "SELECT a.*, s.title, $reviewerfields, $authorfields, $overbyfields
                  FROM {peerreview_assessments} a
            INNER JOIN {user} reviewer ON (a.reviewerid = reviewer.id)
            INNER JOIN {peerreview_submissions} s ON (a.submissionid = s.id AND s.example = 0)
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.id = :sid AND reviewer.id = :rid AND s.peerreviewid = :peerreviewid";
        $params = array('sid' => $submissionid, 'rid' => $reviewerid, 'peerreviewid' => $this->id);
    ...
    public function get_assessments_of_submission($submissionid)
    {
        ...
        $sql = "SELECT a.*, s.title, $reviewerfields, $overbyfields
            ...
            INNER JOIN {peerreview_submissions} s ON (a.submissionid = s.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.example = 0 AND s.id = :submissionid AND s.peerreviewid = :peerreviewid
              ORDER BY $sort";
    ...
    public function get_assessments_by_reviewer($reviewerid)
    {
        ...
        $sql = "SELECT a.*, $reviewerfields, $authorfields, $overbyfields,
            ...
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.example = 0 AND reviewer.id = :reviewerid AND s.peerreviewid = :peerreviewid";
        $params = array('reviewerid' => $reviewerid, 'peerreviewid' => $this->id);
    ...
    public function add_allocation(stdclass $submission, $reviewerid, $weight = 1, $bulk = false)
    {
        ...
        if ($DB->record_exists('peerreview_assessments', array('submissionid' => $submission->id, 'reviewerid' => $reviewerid))) {
        ...
        return $DB->insert_record('peerreview_assessments', $assessment, true, $bulk);
    }

    /**
     * Delete assessment record or records.
     *
     * Removes associated records from the peerreview_grades table, too.
     *
    ...
    public function delete_assessment($id)
    {
        ...
        if (is_array($id)) {
            $DB->delete_records_list('peerreview_grades', 'assessmentid', $id);
            foreach ($id as $itemid) {
                $fs->delete_area_files($this->context->id, 'mod_peerreview', 'overallfeedback_content', $itemid);
                $fs->delete_area_files($this->context->id, 'mod_peerreview', 'overallfeedback_attachment', $itemid);
            }
            $DB->delete_records_list('peerreview_assessments', 'id', $id);
        } else {
            $DB->delete_records('peerreview_grades', array('assessmentid' => $id));
            $fs->delete_area_files($this->context->id, 'mod_peerreview', 'overallfeedback_content', $id);
            $fs->delete_area_files($this->context->id, 'mod_peerreview', 'overallfeedback_attachment', $id);
            $DB->delete_records('peerreview_assessments', array('id' => $id));
        }
    ...
    public function grading_strategy_instance()
    {
        ...
        if (is_null($this->strategyinstance)) {
            ...
            $classname = 'peerreview_' . $this->strategy . '_strategy';
            $this->strategyinstance = new $classname($this);
            if (!in_array('peerreview_strategy', class_implements($this->strategyinstance))) {
                throw new coding_exception($classname . ' does not implement peerreview_strategy interface');
            }
        }
     ...
     *
     * @param string $method the name of the peerrevieweval subplugin
    ...
    public function set_grading_evaluation_method($method)
    {
        ...
        if (is_readable($evaluationlib)) {
            ...
            $DB->set_field('peerreview', 'evaluation', $method, array('id' => $this->id));
            return true;
        }
    ...
    public function grading_evaluation_instance()
    {
        ...
        if (is_null($this->evaluationinstance)) {
            ...
            $classname = 'peerreview_' . $this->evaluation . '_evaluation';
            $this->evaluationinstance = new $classname($this);
            if (!in_array('peerreview_evaluation', class_parents($this->evaluationinstance))) {
                throw new coding_exception($classname . ' does not extend peerreview_evaluation class');
            }
        }
    ...
    public function allocator_instance($method)
    {
        ...
        $classname = 'peerreview_' . $method . '_allocator';
        return new $classname($this);
    }

    /**
     * @return moodle_url of this peerreview's view page
     */
    public function view_url()
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/view.php', array('id' => $this->cm->id));
    }

    /**
     * @return moodle_url of the page for editing this peerreview's grading form
     */
    public function editform_url()
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/editform.php', array('cmid' => $this->cm->id));
    }

    /**
     * @return moodle_url of the page for previewing this peerreview's grading form
     */
    public function previewform_url()
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/editformpreview.php', array('cmid' => $this->cm->id));
    }
    ...
    public function assess_url($assessmentid)
    {
        ...
        return new moodle_url('/mod/peerreview/assessment.php', array('asid' => $assessmentid));
    }
    ...
    public function exassess_url($assessmentid)
    {
        global $CFG;
        $assessmentid = clean_param($assessmentid, PARAM_INT);
        return new moodle_url('/mod/peerreview/exassessment.php', array('asid' => $assessmentid));
    }
    ...
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/submission.php', array('cmid' => $this->cm->id, 'id' => $id));
    }
    ...
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/exsubmission.php', array('cmid' => $this->cm->id, 'id' => $id));
    }
    ...
    public function compare_url($sid, array $aids)
    {
        ...
        $url = new moodle_url('/mod/peerreview/compare.php', array('cmid' => $this->cm->id, 'sid' => $sid));
        $i = 0;
        foreach ($aids as $aid) {
            $url->param("aid{$i}", $aid);
            $i++;
    ...
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/excompare.php', array('cmid' => $this->cm->id, 'sid' => $sid, 'aid' => $aid));
    }
    ...
    public function allocation_url($method = null)
    {
        ...
        return new moodle_url('/mod/peerreview/allocation.php', $params);
    }
    ...
    public function switchphase_url($phasecode)
    {
        ...
        return new moodle_url('/mod/peerreview/switchphase.php', array('cmid' => $this->cm->id, 'phase' => $phasecode));
    }
    ...
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/aggregate.php', array('cmid' => $this->cm->id));
    }

    /**
     * @return moodle_url of this peerreview's toolbox page
     */
    public function toolbox_url($tool)
    {
        global $CFG;
        return new moodle_url('/mod/peerreview/toolbox.php', array('id' => $this->cm->id, 'tool' => $tool));
    }

    /**
     * peerreview wrapper around {@see add_to_log()}
     * @deprecated since 2.7 Please use the provided event classes for logging actions.
     *
     * @param string $action to be logged
     * @param moodle_url $url absolute url as returned by {@see peerreview::submission_url()} and friends
    ...
    public function log($action, moodle_url $url = null, $info = null, $return = false)
    {
        ...
        $args = array($this->course->id, 'peerreview', $action, $logurl, $info, $this->cm->id);
        if ($return) {
            return $args;
        }
    ...
    public function creating_submission_allowed($userid)
    {
        ...
        $now = time();
        $ignoredeadlines = has_capability('mod/peerreview:ignoredeadlines', $this->context, $userid);

        if ($this->latesubmissions) {
            // Edit: Aways able to edit the subission
            ...
            // if (!$ignoredeadlines and $now > $this->submissionend ) {
    ...
    public function assessing_allowed($userid)
    {
        ...
        if ($this->phase != self::PHASE_ASSESSMENT) {
            ...
            if ($this->phase != self::PHASE_EVALUATION or !has_capability('mod/peerreview:overridegrades', $this->context, $userid)) {
                return false;
            }
        }
     ...
     *
     * Returns null if example submissions are not enabled in this peerreview. Otherwise returns
    ...
    /**
     * Switch to a new peerreview phase
     *
    ...
    public function switch_phase($newphase)
    {
        ...
        if (self::PHASE_CLOSED == $newphase) {
            ...
            foreach ($this as $property => $value) {
                $peerreview->{$property} = $value;
            }
            $peerreview->course     = $this->course->id;
            $peerreview->cmidnumber = $this->cm->id;
            $peerreview->modname    = 'peerreview';
            peerreview_update_grades($peerreview);
        }

        $DB->set_field('peerreview', 'phase', $newphase, array('id' => $this->id));
        $this->phase = $newphase;
        $eventdata = array(
            'objectid' => $this->id,
            'context' => $this->context,
            'other' => array(
                'peerreviewphase' => $this->phase
            )
        );
        $event = \mod_peerreview\event\phase_switched::create($eventdata);
        $event->trigger();
        return true;
    }
    ...
    public function set_peer_grade($assessmentid, $grade)
    {
        ...
        $DB->update_record('peerreview_assessments', $data);
        return $grade;
    }

    /**
     * Prepares data object with all peerreview grades to be rendered
     *
    ...
    public function prepare_grading_report_data($userid, $groupid, $page, $perpage, $sortby, $sorthow)
    {
        ...
        $canviewall     = has_capability('mod/peerreview:viewallassessments', $this->context, $userid);
        ...
        if (!in_array($sortby, array(
            'lastname', 'firstname', 'submissiontitle', 'submissionmodified',
        ...
        if ($canviewall) {
            ...
            // this is an ordinary peerreview participant (aka student) - display the report just for him/her
        ...
        if ($numofparticipants > 0) {
            ...
            $params['peerreviewid1'] = $this->id;
            $params['peerreviewid2'] = $this->id;
            $sqlsort = array();
            ...
            $sql = "SELECT $picturefields, s.title AS submissiontitle, s.timemodified AS submissionmodified,
                      ...
                      FROM {user} u
                 LEFT JOIN {peerreview_submissions} s ON (s.authorid = u.id AND s.peerreviewid = :peerreviewid1 AND s.example = 0)
                 LEFT JOIN {peerreview_aggregations} ag ON (ag.userid = u.id AND ag.peerreviewid = :peerreviewid2)
        ...
        if ($submissions) {
            ...
            $sql = "SELECT a.id AS assessmentid, a.submissionid, a.grade, a.gradinggrade, a.gradinggradeover, a.weight,
                      ...
                      JOIN {peerreview_submissions} s ON (a.submissionid = s.id AND s.example = 0)
        ...
        if ($participants) {
            ...
            $sql = "SELECT a.id AS assessmentid, a.submissionid, a.grade, a.gradinggrade, a.gradinggradeover, a.reviewerid, a.weight,
                     ...
                     WHERE u.id $participantids AND s.peerreviewid = :peerreviewid
                  ORDER BY a.weight DESC, $sort";
     ...
     * clears the calculated grades in peerreview_assessments. Therefore reviewers have to re-assess
    ...
    public function clear_assessments()
    {
        ...
        $DB->set_field_select('peerreview_assessments', 'grade', null, $sql, $params);
        $DB->set_field_select('peerreview_assessments', 'gradinggrade', null, $sql, $params);
    }
    ...
    public function clear_submission_grades($restrict = null)
    {
        ...
        $sql = "peerreviewid = :peerreviewid AND example = 0";
        $params = array('peerreviewid' => $this->id);
        ...
        $DB->set_field_select('peerreview_submissions', 'grade', null, $sql, $params);
    }
    ...
    public function aggregate_submission_grades($restrict = null)
    {
        ...
        $sql = 'SELECT s.id AS submissionid, s.grade AS submissiongrade,
             ...
             LEFT JOIN {peerreview_assessments} a ON (a.submissionid = s.id)
                 WHERE s.example=0 AND s.peerreviewid=:peerreviewid'; // to be cont.
        $params = array('peerreviewid' => $this->id);
    ...
    public function clear_grading_grades($restrict = null)
    {
        ...
        $DB->set_field_select('peerreview_aggregations', 'gradinggrade', null, $sql, $params);
    }
    ...
    public function aggregate_grading_grades($restrict = null)
    {
        ...
        $sql = 'SELECT a.reviewerid, a.gradinggrade, a.gradinggradeover,
                  ...
                  FROM {peerreview_assessments} a
            INNER JOIN {peerreview_submissions} s ON (a.submissionid = s.id)
             LEFT JOIN {peerreview_aggregations} ag ON (ag.userid = a.reviewerid AND ag.peerreviewid = s.peerreviewid)
                 WHERE s.example=0 AND s.peerreviewid=:peerreviewid'; // to be cont.
        $params = array('peerreviewid' => $this->id);
     ...
     * @return peerreview_feedbackreviewer_form
     */
    public function get_feedbackreviewer_form($actionurl, stdclass $assessment, $options = array())
    {
        ...
        if (is_null($current->gradinggrade)) {
            $current->gradinggrade = get_string('nullgrade', 'peerreview');
        }
        ...
        return new peerreview_feedbackreviewer_form(
            $actionurl,
            array('peerreview' => $this, 'current' => $current, 'editoropts' => array(), 'options' => $options),
            'post',
            '',
            null,
            $editable
     ...
     * @return peerreview_feedbackauthor_form
     */
    public function get_feedbackauthor_form($actionurl, stdclass $submission, $options = array())
    {
        ...
        if (is_null($current->grade)) {
            $current->grade = get_string('nullgrade', 'peerreview');
        }
        if (!isset($options['editable'])) {
            $editable = true;   // by default
        } else {
        ...
        return new peerreview_feedbackauthor_form(
            $actionurl,
            array('peerreview' => $this, 'current' => $current, 'editoropts' => array(), 'options' => $options),
            'post',
            '',
     ...
     *
     * The submission grade is returned for users with the capability mod/peerreview:submit and the
     * assessment grade is returned for users with the capability mod/peerreview:peerassess. Unless the
     ...
     * @return peerreview_final_grades|false
     */
    public function get_gradebook_grades($userid)
    {
        ...
        $gradebook = grade_get_grades($this->course->id, 'mod', 'peerreview', $this->id, $userid);

        $grades = new peerreview_final_grades();

        if (has_capability('mod/peerreview:submit', $this->context, $userid)) {
        ...
        if (has_capability('mod/peerreview:peerassess', $this->context, $userid)) {
    ...
    /**
     * Performs the reset of this peerreview instance.
     *
    ...
    public function reset_userdata(stdClass $data)
    {
        ...
        $componentstr = get_string('pluginname', 'peerreview') . ': ' . format_string($this->name);
        $status = array();

        if (!empty($data->reset_peerreview_assessments) or !empty($data->reset_peerreview_submissions)) {
            ...
            if ($result === true) {
                $status[] = array(
                    ...
                    'item' => get_string('resetassessments', 'mod_peerreview'),
                    'error' => false,
                );
            } else {
                $status[] = array(
        ...
        if (!empty($data->reset_peerreview_submissions)) {
            ...
            if ($result === true) {
                $status[] = array(
                    'component' => $componentstr,
                    'item' => get_string('resetsubmissions', 'mod_peerreview'),
                    'error' => false,
                );
            } else {
                $status[] = array(
        ...
        if (!empty($data->reset_peerreview_phase)) {
            // Do not use the {@link peerreview::switch_phase()} here, we do not
     ...
     * check the peerreview activity group mode.
     *
     * If the peerreview is not in a group mode, or if it is in the visible group
     * mode, this method returns true. This is consistent with how the
     * {@link groups_get_activity_allowed_groups()} behaves.
     *
     * If the peerreview is in a separate group mode, the current user has to
    ...
    public function check_group_membership($otheruserid)
    {
        ...
        if (groups_get_activity_groupmode($this->cm) != SEPARATEGROUPS) {
            // The peerreview is not in a group mode, or it is in a visible group mode.
            return true;
    ...
    public function check_examples_assessed_before_submission($userid)
    {
        ...
        if (
            $this->useexamples and $this->examplesmode == self::EXAMPLES_BEFORE_SUBMISSION
            and !has_capability('mod/peerreview:manageexamples', $this->context)
        ) {
    ...
    public function check_examples_assessed_before_assessment($userid)
    {
        ...
        if (
            $this->useexamples and $this->examplesmode == self::EXAMPLES_BEFORE_ASSESSMENT
            and !has_capability('mod/peerreview:manageexamples', $this->context)
        ) {
    ...
    public function set_module_viewed()
    {
        ...
        $event = \mod_peerreview\event\course_module_viewed::create($eventdata);
    ...
    public function validate_submission_data($data)
    {
        ...
        if (empty($data['id']) and empty($data['example'])) {
            ...
            $sql = "SELECT COUNT(s.id)
                      FROM {peerreview_submissions} s
                      JOIN {peerreview} w ON (s.peerreviewid = w.id)
                      JOIN {course_modules} cm ON (w.id = cm.instance)
                      JOIN {modules} m ON (m.name = 'peerreview' AND m.id = cm.module)
                     WHERE cm.id = ? AND s.authorid = ? AND s.example = 0";

            if ($DB->count_records_sql($sql, array($data['cmid'], $USER->id))) {
                $errors['title'] = get_string('err_multiplesubmissions', 'mod_peerreview');
            }
        }
        // Get the peerreview record by id or cmid, depending on whether we're creating or editing a submission.
        if (empty($data['peerreviewid'])) {
            $peerreview = $DB->get_record_select(
                'peerreview',
                'id = (SELECT instance FROM {course_modules} WHERE id = ?)',
                [$data['cmid']]
            );
        } else {
            $peerreview = $DB->get_record('peerreview', ['id' => $data['peerreviewid']]);
        }
        ...
        } else if ($peerreview->submissiontypefile == peerreview_SUBMISSION_TYPE_DISABLED && !empty($data['attachment_filemanager'])) {
            $errors['attachment_filemanager'] = get_string('submissiontypedisabled', 'mod_peerreview');
        }

        if ($peerreview->submissiontypetext == peerreview_SUBMISSION_TYPE_REQUIRED && html_is_blank($data['content_editor']['text'])) {
            $errors['content_editor'] = get_string('err_required', 'form');
        } else if ($peerreview->submissiontypetext == peerreview_SUBMISSION_TYPE_DISABLED && !empty($data['content_editor']['text'])) {
            $errors['content_editor'] = get_string('submissiontypedisabled', 'mod_peerreview');
        }

        // If neither type is explicitly required, one or the other must be submitted.
        if (
            $peerreview->submissiontypetext != peerreview_SUBMISSION_TYPE_REQUIRED
            && $peerreview->submissiontypefile != peerreview_SUBMISSION_TYPE_REQUIRED
            && empty($attachments) && html_is_blank($data['content_editor']['text'])
        ) {
            $errors['content_editor'] = get_string('submissionrequiredcontent', 'mod_peerreview');
            $errors['attachment_filemanager'] = get_string('submissionrequiredfile', 'mod_peerreview');
        }

        /* EDIT: add description field */
        if (html_is_blank($data['description_editor']['text'])) {
            $errors['description_editor'] = get_string('err_required', 'form');
        }
        ...
        if (
            $peerreview->submissiontypetext != peerreview_SUBMISSION_TYPE_REQUIRED
            && $peerreview->submissiontypefile != peerreview_SUBMISSION_TYPE_REQUIRED
            && empty($attachments) && html_is_blank($data['description_editor']['text'])
        ) {
            $errors['description_editor'] = get_string('submissionrequiredcontent', 'mod_peerreview');
            $errors['attachment_filemanager'] = get_string('submissionrequiredfile', 'mod_peerreview');
        }
    ...
    public function edit_submission($submission)
    {
        ...
        if (is_null($submission->id)) {
            $submission->peerreviewid     = $this->id;
        ...
        $logdata = null;
        if (is_null($submission->id)) {
            $submission->id = $DB->insert_record('peerreview_submissions', $submission);
            $params['objectid'] = $submission->id;
            $event = \mod_peerreview\event\submission_created::create($params);
            $event->trigger();
        } else {
            if (empty($submission->id) or empty($submission->id) or ($submission->id != $submission->id)) {
                throw new moodle_exception('err_submissionid', 'peerreview');
            }
        }
        $params['objectid'] = $submission->id;

        // Save and relink embedded images and save attachments.
        if ($this->submissiontypetext != peerreview_SUBMISSION_TYPE_DISABLED) {
            $submission = file_postupdate_standard_editor(
                $submission,
                'content',
                $this->submission_content_options(),
                $this->context,
                'mod_peerreview',
                'submission_content',
                $submission->id
        ...
        $submission = file_postupdate_standard_filemanager(
            $submission,
            'attachment',
            $this->submission_attachment_options(),
            $this->context,
            'mod_peerreview',
        ...
        $DB->update_record('peerreview_submissions', $submission);
        $event = \mod_peerreview\event\submission_updated::create($params);
        $event->add_record_snapshot('peerreview', $this->dbrecord);
        $event->trigger();

        // Send submitted content for plagiarism detection.
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'mod_peerreview', 'submission_attachment', $submission->id);

        $params['other']['content'] = $submission->content;
        /* EDIT: add description field */
        $params['other']['description'] = $submission->description;

        $params['other']['pathnamehashes'] = array_keys($files);

        $event = \mod_peerreview\event\assessable_uploaded::create($params);
        $event->trigger();
    ...
    public function check_view_assessment($assessment, $submission)
    {
        ...
        // EDIT: Everyone can view all assessments
        return true;

        $isauthor = $submission->authorid == $USER->id;
        $isreviewer = $assessment->reviewerid == $USER->id;
        $canviewallassessments  = has_capability('mod/peerreview:viewallassessments', $this->context);
        $canviewallsubmissions  = has_capability('mod/peerreview:viewallsubmissions', $this->context);
        ...
        if ($isauthor and !$isreviewer and !$canviewallassessments and $this->phase != self::PHASE_CLOSED) {
            // Authors can see assessments of their work at the end of peerreview only.
            throw new \moodle_exception(
                'nopermissions',
                'error',
                $this->view_url(),
                'view assessment of own work before peerreview is closed'
            );
        }
    }
    ...
    public function check_edit_assessment($assessment, $submission)
    {
        ...
        if (!$assessed) {
            throw new moodle_exception($notice, 'mod_peerreview');
        }
    }
    ...
    public function edit_assessment($assessment, $submission, $data, $strategy)
    {
        ...
        $cansetassessmentweight = has_capability('mod/peerreview:allocate', $this->context);
        ...
        if (isset($data->feedbackauthor_editor)) {
            ...
            $coredata = file_postupdate_standard_editor(
                $coredata,
                'feedbackauthor',
                $this->overall_feedback_content_options(),
                $this->context,
                'mod_peerreview',
        ...
        if (isset($data->anonymousreview)) {
            $coredata->anonymousreview = $data->anonymousreview;
        }
        ...
        if (isset($data->feedbackauthorattachment_filemanager)) {
            ...
            $coredata = file_postupdate_standard_filemanager(
                $coredata,
                'feedbackauthorattachment',
                $this->overall_feedback_attachment_options(),
                $this->context,
                'mod_peerreview',
        ...
        if (count((array)$coredata) > 1) {
            ...
            $params = array(
                ...
                'context' => $this->context,
                'other' => array(
                    'peerreviewid' => $this->id,
                    'submissionid' => $assessment->submissionid,
                    'anonymousreview' => $data->anonymousreview
                )
            );

            if (is_null($assessment->grade)) {
                // All peerreview_assessments are created when allocations are made. The create event is of more use located here.
                $event = \mod_peerreview\event\submission_assessed::create($params);
                $event->trigger();
            } else {
                $params['other']['grade'] = $assessment->grade;
                $event = \mod_peerreview\event\submission_reassessed::create($params);
                $event->trigger();
            }
        }
        return $rawgrade;
    ...
    public function evaluate_assessment($assessment, $data, $cansetassessmentweight, $canoverridegrades)
    {
        ...
        $DB->update_record('peerreview_assessments', $record);
    }
    ...
    public function set_submission_viewed($submission)
    {
        ...
        $event = \mod_peerreview\event\submission_viewed::create($params);
        $event->trigger();
    }
    ...
    public function evaluate_submission($submission, $data, $canpublish, $canoverride)
    {
        ...
        $DB->update_record('peerreview_submissions', $record);
    }
    ...
    public function init_initial_bar(): void
    {
        ...
        if (empty($SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id])) {
            $SESSION->mod_peerreview = new stdClass();
            $SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id] = [];
        }
        if (!empty($SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id]['i_first'])) {
            $this->initialbarprefs['i_first'] = $SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id]['i_first'];
        }
        if (!empty($SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id]['i_last'])) {
            $this->initialbarprefs['i_last'] = $SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id]['i_last'];
        }
        if (!is_null($ifirst)) {
            $this->initialbarprefs['i_first'] = $ifirst;
            $SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id]['i_first'] = $ifirst;
        }

        if (!is_null($ilast)) {
            $this->initialbarprefs['i_last'] = $ilast;
            $SESSION->mod_peerreview->initialbarprefs['id-' . $this->context->id]['i_last'] = $ilast;
        }
    }
    ...
    protected function aggregate_grading_grades_process(array $assessments, $timegraded = null)
    {
        ...
        if (grade_floats_different($finalgrade, $current)) {
            ...
            if (is_null($agid)) {
                ...
                $record = new stdclass();
                $record->peerreviewid = $this->id;
                $record->userid = $reviewerid;
                $record->gradinggrade = $finalgrade;
                $record->timegraded = $timegraded;
                $record->id = $DB->insert_record('peerreview_aggregations', $record);
                $params['objectid'] = $record->id;
                $event = \mod_peerreview\event\assessment_evaluated::create($params);
                $event->trigger();
            } else {
                $record = new stdclass();
                $record->id = $agid;
                $record->gradinggrade = $finalgrade;
                $record->timegraded = $timegraded;
                $DB->update_record('peerreview_aggregations', $record);
                $params['objectid'] = $agid;
                $event = \mod_peerreview\event\assessment_reevaluated::create($params);
                $event->trigger();
            }
        }
    }
    ...
    /**
     * Returns SQL to fetch all enrolled users with the given capability in the current peerreview
     *
    ...
    protected function get_users_with_capability_sql($capability, $musthavesubmission, $groupid)
    {
        ...
        if ($musthavesubmission) {
            $sql .= " JOIN {peerreview_submissions} ws ON (ws.authorid = u.id AND ws.example = 0 AND ws.peerreviewid = :peerreviewid{$inc}) ";
            $params['peerreviewid' . $inc] = $this->id;
        }
    ...
    /**
     * Returns SQL statement that can be used to fetch all actively enrolled participants in the peerreview
     *
     * @param bool $musthavesubmission if true, return only users who have already submitted
     * @param int $groupid 0 means ignore groups, any other value limits the result by group id
     * @return array of (string)sql, (array)params
    ...
    protected function get_participants_sql($musthavesubmission = false, $groupid = 0)
    {
        ...
        list($sql1, $params1) = $this->get_users_with_capability_sql('mod/peerreview:submit', $musthavesubmission, $groupid);
        list($sql2, $params2) = $this->get_users_with_capability_sql('mod/peerreview:peerassess', $musthavesubmission, $groupid);
    ...
    /**
     * @return array of available peerreview phases
     */
    ...
    protected function log_convert_url(moodle_url $fullurl)
    {
        ...
        if (!isset($baseurl)) {
            $baseurl = new moodle_url('/mod/peerreview/');
            $baseurl = $baseurl->out();
        }
    ...
    protected function reset_userdata_assessments(stdClass $data)
    {
        ...
        $sql = "SELECT a.id
                 ...
                 WHERE s.peerreviewid = :peerreviewid
                       AND (s.example = 0 OR (s.example = 1 AND a.weight = 0))";

        $assessments = $DB->get_records_sql($sql, array('peerreviewid' => $this->id));
        $this->delete_assessment(array_keys($assessments));

        $DB->delete_records('peerreview_aggregations', array('peerreviewid' => $this->id));
    ...
    /**
     * Hard set the peerreview phase to the setup one.
     */
    protected function reset_phase()
    {
        global $DB;

        $DB->set_field('peerreview', 'phase', self::PHASE_SETUP, array('id' => $this->id));
     ...
     * @param \stdClass $a
     */
     function peerreview_send_alert($user, $a) {
        global $PAGE;
        ...
        $message->name = 'peernotifications';
        $message->notification = 1;
        $message->userfrom = \core_user::get_noreply_user();
        $message->userto = $user;
        $message->subject = $a->subject;
        $message->fullmessage = $a->fullmessage;
        $message->fullmessageformat = FORMAT_PLAIN;
        $message->fullmessagehtml = $a->fullmessage;
        $message->smallmessage = $a->smallmessage;
        $message->contexturl = $a->contexturl;
        $message->contexturlname = $a->contexturlname;
 ...
 */
class peerreview_user_plan implements renderable
{
    ...
    /**
     * Prepare an individual peerreview plan for the given user.
     *
     * @param peerreview $peerreview instance
     * @param int $userid whom the plan is prepared for
     */
    public function __construct(peerreview $peerreview, $userid)
    {
        global $DB;
        ...
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phasesetup', 'peerreview');
        $phase->tasks = array();
        if (has_capability('moodle/course:manageactivities', $peerreview->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskintro', 'peerreview');
            $task->link = $peerreview->updatemod_url();
            $task->completed = !(trim($peerreview->intro) == '');
            $phase->tasks['intro'] = $task;
        }
        if (has_capability('moodle/course:manageactivities', $peerreview->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskinstructauthors', 'peerreview');
            $task->link = $peerreview->updatemod_url();
            $task->completed = !(trim($peerreview->instructauthors) == '');
            $phase->tasks['instructauthors'] = $task;
        }
        if (has_capability('mod/peerreview:editdimensions', $peerreview->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('editassessmentform', 'peerreview');
            $task->link = $peerreview->editform_url();
            if ($peerreview->grading_strategy_instance()->form_ready()) {
                $task->completed = true;
            } elseif ($peerreview->phase > peerreview::PHASE_SETUP) {
                $task->completed = false;
            }
            $phase->tasks['editform'] = $task;
        }
        if ($peerreview->useexamples and has_capability('mod/peerreview:manageexamples', $peerreview->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('prepareexamples', 'peerreview');
            if ($DB->count_records('peerreview_submissions', array('example' => 1, 'peerreviewid' => $peerreview->id)) > 0) {
                $task->completed = true;
            } elseif ($peerreview->phase > peerreview::PHASE_SETUP) {
                $task->completed = false;
            }
        ...
        if (empty($phase->tasks) and $peerreview->phase == peerreview::PHASE_SETUP) {
            // if we are in the setup phase and there is no task (typical for students), let us
            // display some explanation what is going on
            $task = new stdclass();
            $task->title = get_string('undersetup', 'peerreview');
            $task->completed = 'info';
            $phase->tasks['setupinfo'] = $task;
        }
        $this->phases[peerreview::PHASE_SETUP] = $phase;

        //---------------------------------------------------------
        // setup | * SUBMISSION | assessment | evaluation | closed
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phasesubmission', 'peerreview');
        $phase->tasks = array();
        if (has_capability('moodle/course:manageactivities', $peerreview->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskinstructreviewers', 'peerreview');
            $task->link = $peerreview->updatemod_url();
            if (trim($peerreview->instructreviewers)) {
                $task->completed = true;
            } elseif ($peerreview->phase >= peerreview::PHASE_ASSESSMENT) {
                $task->completed = false;
            }
            $phase->tasks['instructreviewers'] = $task;
        }
        if (
            $peerreview->useexamples and $peerreview->examplesmode == peerreview::EXAMPLES_BEFORE_SUBMISSION
            and has_capability('mod/peerreview:submit', $peerreview->context, $userid, false)
            and !has_capability('mod/peerreview:manageexamples', $peerreview->context, $userid)
        ) {
            $task = new stdclass();
            $task->title = get_string('exampleassesstask', 'peerreview');
            ...
            $task->details = get_string('exampleassesstaskdetails', 'peerreview', $a);
        ...
        if (has_capability('mod/peerreview:submit', $peerreview->context, $userid, false)) {
            $task = new stdclass();
            $task->title = get_string('tasksubmit', 'peerreview');
            $task->link = $peerreview->submission_url();
            if ($DB->record_exists('peerreview_submissions', array('peerreviewid' => $peerreview->id, 'example' => 0, 'authorid' => $userid))) {
                $task->completed = true;
            } elseif ($peerreview->phase >= peerreview::PHASE_ASSESSMENT) {
                $task->completed = false;
            } else {
        ...
        if (has_capability('mod/peerreview:allocate', $peerreview->context, $userid)) {
            if ($peerreview->phaseswitchassessment) {
                $task = new stdClass();
                $allocator = $DB->get_record('peerreviewallocation_scheduled', array('peerreviewid' => $peerreview->id));
                if (empty($allocator)) {
                    $task->completed = false;
                ...
                $task->title = get_string('setup', 'peerreviewallocation_scheduled');
                $task->link = $peerreview->allocation_url('scheduled');
                $phase->tasks['allocatescheduled'] = $task;
            }
            $task = new stdclass();
            $task->title = get_string('allocate', 'peerreview');
            $task->link = $peerreview->allocation_url();
            $numofauthors = $peerreview->count_potential_authors(false);
            $numofsubmissions = $DB->count_records('peerreview_submissions', array('peerreviewid' => $peerreview->id, 'example' => 0));
            $sql = 'SELECT COUNT(s.id) AS nonallocated
                      FROM {peerreview_submissions} s
                 LEFT JOIN {peerreview_assessments} a ON (a.submissionid=s.id)
                     WHERE s.peerreviewid = :peerreviewid AND s.example=0 AND a.submissionid IS NULL';
            $params['peerreviewid'] = $peerreview->id;
            $numnonallocated = $DB->count_records_sql($sql, $params);
            if ($numofsubmissions == 0) {
                $task->completed = null;
            } elseif ($numnonallocated == 0) {
                $task->completed = true;
            } elseif ($peerreview->phase > peerreview::PHASE_SUBMISSION) {
                $task->completed = false;
            } else {
            ...
            $task->details  = get_string('allocatedetails', 'peerreview', $a);
            unset($a);
            $phase->tasks['allocate'] = $task;

            if ($numofsubmissions < $numofauthors and $peerreview->phase >= peerreview::PHASE_SUBMISSION) {
                $task = new stdclass();
                $task->title = get_string('someuserswosubmission', 'peerreview');
                $task->completed = 'info';
        ...
        if ($peerreview->submissionstart) {
            $task = new stdclass();
            $task->title = get_string('submissionstartdatetime', 'peerreview', peerreview::timestamp_formats($peerreview->submissionstart));
            $task->completed = 'info';
            $phase->tasks['submissionstartdatetime'] = $task;
        }
        if ($peerreview->submissionend) {
            $task = new stdclass();
            $task->title = get_string('submissionenddatetime', 'peerreview', peerreview::timestamp_formats($peerreview->submissionend));
            $task->completed = 'info';
            $phase->tasks['submissionenddatetime'] = $task;
        }
        if (($peerreview->submissionstart < time()) and $peerreview->latesubmissions) {
            ...
            if (
                has_capability('mod/peerreview:switchphase', $peerreview->context, $userid) ||
                (!$peerreview->get_submission_by_author($userid) && $peerreview->submissionend < time())
            ) {
                $task = new stdclass();
                $task->title = get_string('latesubmissionsallowed', 'peerreview');
                $task->completed = 'info';
        ...
        if (isset($phase->tasks['submissionstartdatetime']) or isset($phase->tasks['submissionenddatetime'])) {
            if (has_capability('mod/peerreview:ignoredeadlines', $peerreview->context, $userid)) {
                $task = new stdclass();
                $task->title = get_string('deadlinesignored', 'peerreview');
                $task->completed = 'info';
                $phase->tasks['deadlinesignored'] = $task;
            }
        }
        $this->phases[peerreview::PHASE_SUBMISSION] = $phase;

        //---------------------------------------------------------
        // setup | submission | * ASSESSMENT | evaluation | closed
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phaseassessment', 'peerreview');
        $phase->tasks = array();
        $phase->isreviewer = has_capability('mod/peerreview:peerassess', $peerreview->context, $userid);
        if (
            $peerreview->phase == peerreview::PHASE_SUBMISSION and $peerreview->phaseswitchassessment
            and has_capability('mod/peerreview:switchphase', $peerreview->context, $userid)
        ) {
            $task = new stdClass();
            $task->title = get_string('switchphase30auto', 'mod_peerreview', peerreview::timestamp_formats($peerreview->submissionend));
            $task->completed = 'info';
            $phase->tasks['autoswitchinfo'] = $task;
        }
        if (
            $peerreview->useexamples and $peerreview->examplesmode == peerreview::EXAMPLES_BEFORE_ASSESSMENT
            and $phase->isreviewer and !has_capability('mod/peerreview:manageexamples', $peerreview->context, $userid)
        ) {
            $task = new stdclass();
            $task->title = get_string('exampleassesstask', 'peerreview');
            $examples = $peerreview->get_examples_for_reviewer($userid);
            $a = new stdclass();
            $a->expected = count($examples);
            $a->assessed = 0;
            foreach ($examples as $exampleid => $example) {
        ...
        if (empty($phase->tasks['examples']) or !empty($phase->tasks['examples']->completed)) {
            $phase->assessments = $peerreview->get_assessments_by_reviewer($userid);
            ...
            if ($numofpeers) {
                ...
                $task->title = get_string('taskassesspeers', 'peerreview');
                $task->details = get_string('taskassesspeersdetails', 'peerreview', $a);
                unset($a);
                $phase->tasks['assesspeers'] = $task;
            }
            if ($peerreview->useselfassessment and $numofself) {
                $task = new stdclass();
                if ($numofselftodo == 0) {
                    $task->completed = true;
                } elseif ($peerreview->phase > peerreview::PHASE_ASSESSMENT) {
                ...
                $task->title = get_string('taskassessself', 'peerreview');
                $phase->tasks['assessself'] = $task;
            }
        }
        if ($peerreview->assessmentstart) {
            $task = new stdclass();
            $task->title = get_string('assessmentstartdatetime', 'peerreview', peerreview::timestamp_formats($peerreview->assessmentstart));
            $task->completed = 'info';
            $phase->tasks['assessmentstartdatetime'] = $task;
        }
        if ($peerreview->assessmentend) {
            $task = new stdclass();
            $task->title = get_string('assessmentenddatetime', 'peerreview', peerreview::timestamp_formats($peerreview->assessmentend));
            $task->completed = 'info';
        ...
        $this->phases[peerreview::PHASE_ASSESSMENT] = $phase;

        //---------------------------------------------------------
        // setup | submission | assessment | * EVALUATION | closed
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phaseevaluation', 'peerreview');
        $phase->tasks = array();
        if (has_capability('mod/peerreview:overridegrades', $peerreview->context)) {
            $expected = $peerreview->count_potential_authors(false);
            $calculated = $DB->count_records_select(
                'peerreview_submissions',
                'peerreviewid = ? AND (grade IS NOT NULL OR gradeover IS NOT NULL)',
                array($peerreview->id)
            );
            $task = new stdclass();
            $task->title = get_string('calculatesubmissiongrades', 'peerreview');
            $a = new stdclass();
            $a->expected    = $expected;
            $a->calculated  = $calculated;
            $task->details  = get_string('calculatesubmissiongradesdetails', 'peerreview', $a);
            if ($calculated >= $expected) {
                $task->completed = true;
            } elseif ($peerreview->phase > peerreview::PHASE_EVALUATION) {
                $task->completed = false;
            }
            $phase->tasks['calculatesubmissiongrade'] = $task;

            $expected = $peerreview->count_potential_reviewers(false);
            $calculated = $DB->count_records_select(
                'peerreview_aggregations',
                'peerreviewid = ? AND gradinggrade IS NOT NULL',
                array($peerreview->id)
            );
            $task = new stdclass();
            $task->title = get_string('calculategradinggrades', 'peerreview');
            $a = new stdclass();
            $a->expected    = $expected;
            $a->calculated  = $calculated;
            $task->details  = get_string('calculategradinggradesdetails', 'peerreview', $a);
            if ($calculated >= $expected) {
                $task->completed = true;
            } elseif ($peerreview->phase > peerreview::PHASE_EVALUATION) {
                $task->completed = false;
            ...
            $task = new stdclass();
            $task->title = get_string('evaluategradeswait', 'peerreview');
            $task->completed = 'info';
        ...
        if (has_capability('moodle/course:manageactivities', $peerreview->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskconclusion', 'peerreview');
            $task->link = $peerreview->updatemod_url();
        ...
        $this->phases[peerreview::PHASE_EVALUATION] = $phase;

        //---------------------------------------------------------
        // setup | submission | assessment | evaluation | * CLOSED
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phaseclosed', 'peerreview');
        $phase->tasks = array();
        $this->phases[peerreview::PHASE_CLOSED] = $phase;
        ...
        if (has_capability('mod/peerreview:switchphase', $peerreview->context, $userid)) {
            $nextphases = array(
                peerreview::PHASE_SETUP => peerreview::PHASE_SUBMISSION,
                peerreview::PHASE_SUBMISSION => peerreview::PHASE_ASSESSMENT,
                peerreview::PHASE_ASSESSMENT => peerreview::PHASE_EVALUATION,
                peerreview::PHASE_EVALUATION => peerreview::PHASE_CLOSED,
            );
            foreach ($this->phases as $phasecode => $phase) {
                if ($phase->active) {
                    if (isset($nextphases[$peerreview->phase])) {
                        $task = new stdClass();
                        $task->title = get_string('switchphasenext', 'mod_peerreview');
                        $task->link = $peerreview->switchphase_url($nextphases[$peerreview->phase]);
                        $task->details = '';
                        $task->completed = null;
                    ...
                    $action->url  = $peerreview->switchphase_url($phasecode);
     ...
     *
     * @see peerreview::get_examples_for_reviewer() for the format of returned value
     * @return array
     */
    public function get_examples()
    {
        if (is_null($this->examples)) {
            $this->examples = $this->peerreview->get_examples_for_reviewer($this->userid);
        }
 ...
 * peerreview_submissions table (as returned by {@see peerreview::get_submission_by_id()}
 * for example) into renderable objects.
 */
abstract class peerreview_submission_base
{

    /** @var bool is the submission anonymous (i.e. contains author information) */
    protected $anonymous;

    /* @var array of columns from peerreview_submissions that are assigned as properties */
     ...
     *
     * @param peerreview $peerreview
     * @param stdClass $submission full record
     * @param bool $showauthor show the author-related information
     * @param array $options additional properties
     */
    public function __construct(peerreview $peerreview, stdClass $submission, $showauthor = false)
    {
 ...
 *
 * @see peerreview_renderer::render_peerreview_submission_summary
 */
class peerreview_submission_summary extends peerreview_submission_base implements renderable
{
 ...
 *
 * @see peerreview_renderer::render_peerreview_submission()
 */
class peerreview_submission extends peerreview_submission_summary implements renderable
{
 ...
 *
 * @see peerreview::prepare_example_summary()
 * @see peerreview_renderer::render_peerreview_example_submission_summary()
 */
class peerreview_example_submission_summary extends peerreview_submission_base implements renderable
{
 ...
 *
 * @see peerreview_renderer::render_peerreview_example_submission()
 */
class peerreview_example_submission extends peerreview_example_submission_summary implements renderable
{
     ...
     * of instances of this class
     */
    protected $fields = array('id', 'title', 'content', 'contentformat', 'contenttrust', 'description', 'descriptionformat', 'descriptiontrust', 'attachment');
}
 ...
 * peerreview_assessments table (as returned by {@see peerreview::get_assessment_by_id()}
 * for example) into renderable objects.
 */
abstract class peerreview_assessment_base
{

    /** @var string the optional title of the assessment */
    public $title = '';

    /** @var peerreview_assessment_form $form as returned by {@link peerreview_strategy::get_assessment_form()} */
    public $form;
     ...
     * @param array $options additional properties
     */
    public function __construct(peerreview $peerreview, stdClass $record, array $options = array())
    {
 ...
 */
class peerreview_assessment extends peerreview_assessment_base implements renderable
{
    ...
    /** @var array */
    protected $fields = array(
        'id', 'submissionid', 'weight', 'timecreated',
        'timemodified', 'grade', 'gradinggrade', 'gradinggradeover', 'feedbackauthor',
        'feedbackauthorformat', 'feedbackauthorattachment', 'anonymousreview'
    );
    ...
    public function get_overall_feedback_content()
    {
        ...
        if ($this->peerreview->overallfeedbackmode == 0) {
            return false;
        }
        ...
        $content = format_text(
            ...
            array('overflowdiv' => true, 'context' => $this->peerreview->context)
        );
    ...
    public function get_overall_feedback_attachments()
    {
        ...
        if ($this->peerreview->overallfeedbackfiles == 0) {
            return false;
        }
        ...
        $attachments = array();
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->peerreview->context->id, 'mod_peerreview', 'overallfeedback_attachment', $this->id);
        foreach ($files as $file) {
            ...
            $previewurl = new moodle_url(moodle_url::make_pluginfile_url(
                $this->peerreview->context->id,
                'mod_peerreview',
                'overallfeedback_attachment',
                $this->id,
 ...
 */
class peerreview_example_assessment extends peerreview_assessment implements renderable
{
 ...
 */
class peerreview_example_reference_assessment extends peerreview_assessment implements renderable
{
 ...
 *
 * @see peerreview::renderer::render_peerreview_message()
 */
class peerreview_message implements renderable
{
 ...
 */
class peerreview_grading_report implements renderable
{

    /** @var stdClass returned by {@see peerreview::prepare_grading_report_data()} */
    protected $data;
     ...
     * (in which later case, the [mod_peerreview,nullgrade] string shall be displayed)
     *
     * @param stdClass $data prepared by {@link peerreview::prepare_grading_report_data()}
 ...
 */
abstract class peerreview_feedback
{
 ...
 */
class peerreview_feedback_author extends peerreview_feedback implements renderable
{
 ...
 */
class peerreview_feedback_reviewer extends peerreview_feedback implements renderable
{
 ...
 */
class peerreview_final_grades implements renderable
{
