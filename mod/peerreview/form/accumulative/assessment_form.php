 ...
 * @uses moodleform
 */
class peerreview_accumulative_assessment_form extends peerreview_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'peerreviewform_accumulative', $i+1);
            ...
            $label = get_string('dimensiongradefor', 'peerreviewform_accumulative', $dimtitle);
            if ($fields->{'grade__idx_' . $i}) {
                ...
                $mform->addRule('grade__idx_' . $i, get_string('required'), 'required', null, 'client'); 
                $mform->addRule(array('grade__idx_' . $i, 'minusone'),
                    get_string('mustchoosegrade', 'peerreviewform_accumulative'), 'compare', 'gt');
            }
            // comment
            $label = get_string('dimensioncommentfor', 'peerreviewform_accumulative', $dimtitle);
            ...
            $mform->addRule('peercomment__idx_' . $i, get_string('required'), 'required', null, 'client');
        }
