...
/**
 * Provides the class {@link peerreviewform_accumulative\privacy\provider}
 *
...
class provider implements \core_privacy\local\metadata\null_provider, \mod_peerreview\privacy\peerreviewform_provider {
    ...
    public static function export_assessment_form(\stdClass $user, \context $context, array $subcontext, int $assessmentid) {
        ...
        $sql = "SELECT dim.id, dim.description, dim.descriptionformat, dim.grade AS dimgrade, dim.weight,
                  ...
                  JOIN {peerreviewform_accumulative} dim ON dim.peerreviewid = w.id
        ...
        foreach ($DB->get_records_sql($sql, $params) as $record) {
            ...
            $record->description = $writer->rewrite_pluginfile_urls($subcontext, 'peerreviewform_accumulative',
        ...
        if ($hasdata) {
            ...
            foreach ($dimensionids as $dimensionid) {
                $writer->export_area_files($subcontext, 'peerreviewform_accumulative', 'description', $dimensionid);
            }
        }
    }
}
