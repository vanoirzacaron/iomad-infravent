 ...
 */
class moodle1_peerreviewform_accumulative_handler extends moodle1_peerreviewform_handler {

    /** @var used by {@link self::migrate_legacy_scales()} */
    private $newscaleids = array();

    /**
     * Converts <ELEMENT> into <peerreviewform_accumulative_dimension>
     */
    public function process_legacy_element(array $data, array $raw) {
        ...
        $converted = (array)peerreviewform_accumulative_upgrade_element($fakerecord, $newscaleid, 12345678);
        unset($converted['peerreviewid']);

        $converted['id'] = $data['id'];
        $this->write_xml('peerreviewform_accumulative_dimension', $converted, array('/peerreviewform_accumulative_dimension/id'));

        return $converted;
    }

    /**
     * If needed, creates new standard (global) scale to replace the legacy peerreview one and returns the mapping
     *
     * If the given $oldscaleid represents a scale, returns array $oldscaleid => $newscaleid that
     * can be used as a parameter for {@link peerreviewform_accumulative_upgrade_element()}. Otherwise
     * this method returns empty array.
     *
     * In peerreview 1.x, scale field in peerreview_elements had the following meaning:
     ...
     *   9 | Score out of 100
     *
     * @see peerreviewform_accumulative_upgrade_scales()
    ...
    protected function get_new_scaleid($oldscaleid) {
        ...
        if ($oldscaleid >= 0 and $oldscaleid <= 6) {
            ...
            if (!isset($this->newscaleids[$oldscaleid])) {
                ...
                // inform the peerreview instance that it should annotate the new scale
    ...
    /**
     * Returns a definition of a legacy peerreview scale
     *
     * @see peerreviewform_accumulative_upgrade_scales
    ...
    private function get_new_scale_definition($oldscaleid) {
        ...
        switch ($oldscaleid) {
        case 0:
            $data['name']  = get_string('scalename0', 'peerreviewform_accumulative');
            $data['scale'] = implode(',', array(get_string('no'), get_string('yes')));
            break;
        case 1:
            $data['name']  = get_string('scalename1', 'peerreviewform_accumulative');
            $data['scale'] = implode(',', array(get_string('absent', 'peerreviewform_accumulative'),
                                                get_string('present', 'peerreviewform_accumulative')));
            break;
        case 2:
            $data['name']  = get_string('scalename2', 'peerreviewform_accumulative');
            $data['scale'] = implode(',', array(get_string('incorrect', 'peerreviewform_accumulative'),
                                                get_string('correct', 'peerreviewform_accumulative')));
            break;
        case 3:
            $data['name']  = get_string('scalename3', 'peerreviewform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('poor', 'peerreviewform_accumulative'),
                                                '**',
                                                '*** ' . get_string('good', 'peerreviewform_accumulative')));
            break;
        case 4:
            $data['name']  = get_string('scalename4', 'peerreviewform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'peerreviewform_accumulative'),
                                                '**',
                                                '***',
                                                '**** ' . get_string('excellent', 'peerreviewform_accumulative')));
            break;
        case 5:
            $data['name']  = get_string('scalename5', 'peerreviewform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'peerreviewform_accumulative'),
                                                '**',
                                                '***',
                                                '****',
            ...
            break;
        case 6:
            $data['name']  = get_string('scalename6', 'peerreviewform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'peerreviewform_accumulative'),
                                                '**',
                                                '***',
                                                '****',
...
/**
 * Transforms a given record from peerreview_elements_old into an object to be saved into peerreviewform_accumulative
 *
 ...
 * @return stdclass to be saved in peerreviewform_accumulative
 */
function peerreviewform_accumulative_upgrade_element(stdclass $old, array $newscaleids, $newpeerreviewid) {
    $new = new stdclass();
