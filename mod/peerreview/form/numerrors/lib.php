 ...
 * @return bool
 */
function peerreviewform_numerrors_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('peerreviewform_numerrors', array('id' => $itemid ,'peerreviewid' => $peerreview->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/peerreviewform_numerrors/$filearea/$itemid/$relativepath";
 ...
 */
class peerreview_numerrors_strategy implements peerreview_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'peerreviewform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peerreview_edit_numerrors_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $peerreviewid = $data->peerreviewid;
        $norepeats  = $data->norepeats;

        $data       = $this->prepare_database_fields($data);
        $records    = $data->numerrors; // data to be saved into {peerreviewform_numerrors}
        $mappings   = $data->mappings;  // data to be saved into {peerreviewform_numerrors_map}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('peerreviewform_numerrors', $record);
            } else {
                // exiting field
                $DB->update_record('peerreviewform_numerrors', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts, $PAGE->context,
                                                      'peerreviewform_numerrors', 'description', $record->id);
            $DB->update_record('peerreviewform_numerrors', $record);
        }
        ...
        foreach ($data->mappings as $nonegative => $grade) {
            ...
            if (isset($this->mappings[$nonegative])) {
                $DB->set_field('peerreviewform_numerrors_map', 'grade', $grade,
                            array('peerreviewid' => $this->peerreview->id, 'nonegative' => $nonegative));
            } else {
                $DB->insert_record('peerreviewform_numerrors_map',
                            (object)array('peerreviewid' => $this->peerreview->id, 'nonegative' => $nonegative, 'grade' => $grade));
            }
        }
        ...
        $sql = "DELETE FROM {peerreviewform_numerrors_map}
                      WHERE (($insql nonegative > :maxnonegative) AND (peerreviewid = :peerreviewid))";
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'peerreviewform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peerreview_numerrors_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function get_dimensions_info() {
        ...
        $dimrecords = $DB->get_records('peerreviewform_numerrors', array('peerreviewid' => $this->peerreview->id), 'sort', 'id,weight');
    ...
    public static function delete_instance($peerreviewid) {
        ...
        $DB->delete_records('peerreviewform_numerrors', array('peerreviewid' => $peerreviewid));
        $DB->delete_records('peerreviewform_numerrors_map', array('peerreviewid' => $peerreviewid));
    }
    ...
    protected function load_fields() {
        ...
        $sql = 'SELECT *
                  FROM {peerreviewform_numerrors}
                 WHERE peerreviewid = :peerreviewid
                 ORDER BY sort';
        $params = array('peerreviewid' => $this->peerreview->id);
    ...
    protected function load_mappings() {
        global $DB;
        return $DB->get_records('peerreviewform_numerrors_map', array('peerreviewid' => $this->peerreview->id), 'nonegative',
                                'nonegative,grade'); // we can use nonegative as key here as it must be unique within peerreview
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        $fs         = get_file_storage();
        foreach ($ids as $id) {
            $fs->delete_area_files($PAGE->context->id, 'peerreviewform_numerrors', 'description', $id);
        }
        $DB->delete_records_list('peerreviewform_numerrors', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link peerreview_edit_numerrors_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->numerrors    = array();          // to be stored in {peerreviewform_numerrors}
        $cook->mappings     = array();          // to be stored in {peerreviewform_numerrors_map}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->numerrors[$i]                        = new stdclass();
            $cook->numerrors[$i]->id                    = $raw->{'dimensionid__idx_'.$i};
            $cook->numerrors[$i]->peerreviewid            = $this->peerreview->id;
     ...
     * @return array of filtered records from the table peerreview_grades
     */
