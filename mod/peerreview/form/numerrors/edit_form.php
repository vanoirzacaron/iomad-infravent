 ...
 * @uses moodleform
 */
class peerreview_edit_numerrors_strategy_form extends peerreview_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $plugindefaults     = get_config('peerreviewform_numerrors');
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'peerreviewform_numerrors', $i+1));
            $mform->addElement('hidden', 'dimensionid__idx_'.$i);   // the id in peerreview_forms
            $mform->setType('dimensionid__idx_'.$i, PARAM_INT);
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                    get_string('dimensiondescription', 'peerreviewform_numerrors'), '', $descriptionopts);
            $mform->addElement('text', 'grade0__idx_'.$i, get_string('grade0', 'peerreviewform_numerrors'), array('size'=>'15'));
            $mform->setDefault('grade0__idx_'.$i, $plugindefaults->grade0);
            $mform->setType('grade0__idx_'.$i, PARAM_TEXT);
            $mform->addElement('text', 'grade1__idx_'.$i, get_string('grade1', 'peerreviewform_numerrors'), array('size'=>'15'));
            ...
            $mform->addElement('select', 'weight__idx_'.$i,
                    get_string('dimensionweight', 'peerreviewform_numerrors'), peerreview::available_dimension_weights_list());
            $mform->setDefault('weight__idx_'.$i, 1);
        }

        $mform->addElement('header', 'mappingheader', get_string('grademapping', 'peerreviewform_numerrors'));
        $mform->addElement('static', 'mappinginfo', get_string('maperror', 'peerreviewform_numerrors'),
                                                            get_string('mapgrade', 'peerreviewform_numerrors'));
        ...
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'peerreviewform_numerrors',
                peerreview_numerrors_strategy::ADDDIMS));
