 ...
 * @uses moodleform
 */
class peerreview_edit_rubric_strategy_form extends peerreview_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'peerreviewform_rubric', $i+1));
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                                get_string('dimensiondescription', 'peerreviewform_rubric'), '', $descriptionopts);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $mform->addGroup($levelgrp, 'level__idx_'.$i.'__idy_'.$j, get_string('levelgroup', 'peerreviewform_rubric'), array(' '), false);
        ...
        $mform->addElement('submit', 'adddims', get_string('addmoredimensions', 'peerreviewform_rubric',
                peerreview_rubric_strategy::ADDDIMS));
        $mform->closeHeaderBefore('adddims');

        $mform->addElement('header', 'configheader', get_string('configuration', 'peerreviewform_rubric'));
        $layoutgrp = array();
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutlist', 'peerreviewform_rubric'), 'list');
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutgrid', 'peerreviewform_rubric'), 'grid');
        $mform->addGroup($layoutgrp, 'layoutgrp', get_string('layout', 'peerreviewform_rubric'), array('<br />'), false);
            ...
            for ($j = 0; isset($data['levelid__idx_'.$i.'__idy_'.$j]); $j++) {
                ...
                if (isset($dimgrades[$levelgrade])) {
                    ...
                    $errors['level__idx_'.$i.'__idy_'.$j] = $errors['level__idx_'.$i.'__idy_'.$k] = get_string('mustbeunique',
                        'peerreviewform_rubric');
                } else {
                    $dimgrades[$levelgrade] = $j;
                }
            }
            if (!$atleastonelevel) {
                $errors['level__idx_'.$i.'__idy_0'] = get_string('mustdefinelevel', 'peerreviewform_rubric');
            }
        }
