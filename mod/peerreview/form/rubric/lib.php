 ...
 * @return bool
 */
function peerreviewform_rubric_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('peerreviewform_rubric', array('id' => $itemid ,'peerreviewid' => $peerreview->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/peerreviewform_rubric/$filearea/$itemid/$relativepath";
 ...
 */
class peerreview_rubric_strategy implements peerreview_strategy {
     ...
     * Constructor
     *
     * @param peerreview $peerreview The peerreview instance record
     * @return void
     */
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        $this->descriptionopts = array_merge(array('context' => $this->peerreview->context), $this->descriptionopts);
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $this->peerreview->context, 'peerreviewform_rubric', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peerreview_edit_rubric_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        if ($DB->record_exists('peerreviewform_rubric_config', array('peerreviewid' => $this->peerreview->id))) {
            $DB->set_field('peerreviewform_rubric_config', 'layout', $layout, array('peerreviewid' => $this->peerreview->id));
        } else {
            $record = new stdclass();
            $record->peerreviewid = $this->peerreview->id;
            $record->layout     = $layout;
            $DB->insert_record('peerreviewform_rubric_config', $record, false);
        }
        ...
        foreach ($data as $record) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('peerreviewform_rubric', $record);
            } else {
                // exiting field
                $DB->update_record('peerreviewform_rubric', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $this->peerreview->context, 'peerreviewform_rubric', 'description', $record->id);
            $DB->update_record('peerreviewform_rubric', $record);
            ...
            foreach ($record->levels as $level) {
                ...
                if (empty($level->id)) {
                    // new field
                    $level->id = $DB->insert_record('peerreviewform_rubric_levels', $level);
                } else {
                    // exiting field
                    $DB->update_record('peerreviewform_rubric_levels', $level);
                }
            }
        }
        $DB->delete_records_list('peerreviewform_rubric_levels', 'id', $deletelevs);
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $this->peerreview->context->id, 'peerreviewform_rubric', 'description',
        ...
        $formclassname = 'peerreview_rubric_' . $this->config->layout . '_assessment_form';
    ...
    public function save_assessment(stdclass $assessment, stdclass $data) {
        ...
        for ($i = 0; isset($data->{'dimensionid__idx_' . $i}); $i++) {
            ...
            if (empty($grade->id)) {
                // new grade
                $grade->id = $DB->insert_record('peerreview_grades', $grade);
            } else {
                // updated grade
                $DB->update_record('peerreview_grades', $grade);
            }
        }
    ...
    public function get_assessments_recordset($restrict=null) {
        ...
        $sql = 'SELECT s.id AS submissionid,
                  ...
                  JOIN {peerreview_assessments} a ON (a.submissionid = s.id)
                  JOIN {peerreview_grades} g ON (g.assessmentid = a.id AND g.strategy = :strategy)
                 WHERE s.example=0 AND s.peerreviewid=:peerreviewid'; // to be cont.
        $params = array('peerreviewid' => $this->peerreview->id, 'strategy' => $this->peerreview->strategy);
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id AS id, MIN(l.grade) AS min, MAX(l.grade) AS max, 1 AS weight
                  FROM {peerreviewform_rubric} d
            INNER JOIN {peerreviewform_rubric_levels} l ON (d.id = l.dimensionid)
                 WHERE d.peerreviewid = :peerreviewid
              GROUP BY d.id';
        $params = array('peerreviewid' => $this->peerreview->id);
     ...
     * @param int|null $peerreviewid id of peerreview instance to check, checks all in case of null
     * @return bool
     */
    public static function scale_used($scaleid, $peerreviewid=null) {
        return false;
    }
    ...
    public static function delete_instance($peerreviewid) {
        ...
        $dimensions = $DB->get_records('peerreviewform_rubric', array('peerreviewid' => $peerreviewid), '', 'id');
        $DB->delete_records_list('peerreviewform_rubric_levels', 'dimensionid', array_keys($dimensions));
        $DB->delete_records('peerreviewform_rubric', array('peerreviewid' => $peerreviewid));
        $DB->delete_records('peerreviewform_rubric_config', array('peerreviewid' => $peerreviewid));
    }
    ...
    /**
     * Loads the fields of the assessment form currently used in this peerreview
     *
    ...
    protected function load_fields() {
        ...
        $sql = "SELECT r.id AS rid, r.sort, r.description, r.descriptionformat,
                  ...
                  FROM {peerreviewform_rubric} r
             LEFT JOIN {peerreviewform_rubric_levels} l ON (l.dimensionid = r.id)
                 WHERE r.peerreviewid = :peerreviewid
    ...
    protected function load_config() {
        ...
        if (!$config = $DB->get_record('peerreviewform_rubric_config', array('peerreviewid' => $this->peerreview->id), 'layout')) {
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($this->peerreview->context->id, 'peerreviewform_rubric', 'description', $id);
            }
        }
        $DB->delete_records_list('peerreviewform_rubric', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link peerreview_edit_rubric_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        for ($i = 0; $i < $raw->norepeats; $i++) {
            ...
            $cook[$i]->peerreviewid           = $this->peerreview->id;
            $cook[$i]->sort                 = $i + 1;
     ...
     * @return array [int dimensionid] => stdclass peerreview_grades record
     */
    ...
    protected function update_peer_grade(stdclass $assessment) {
        ...
        if (!is_null($suggested)) {
            $this->peerreview->set_peer_grade($assessment->id, $suggested);
        }
        return $suggested;
    }
