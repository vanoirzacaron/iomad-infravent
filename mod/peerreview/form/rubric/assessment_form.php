 ...
 */
abstract class peerreview_rubric_assessment_form extends peerreview_assessment_form {
    ...
    public function validation($data, $files) {
        ...
        for ($i = 0; isset($data['dimensionid__idx_'.$i]); $i++) {
            if (empty($data['chosenlevelid__idx_'.$i])) {
                $errors['chosenlevelid__idx_'.$i] = get_string('mustchooseone', 'peerreviewform_rubric'); // used in grid
                $errors['levelgrp__idx_'.$i] = get_string('mustchooseone', 'peerreviewform_rubric');      // used in list
            }
        }
        return $errors;
    }
 ...
 */
class peerreview_rubric_list_assessment_form extends peerreview_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'peerreviewform_rubric', $i+1);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $levelgrp[] = $mform->createElement('radio', 'chosenlevelid__idx_'.$i, '',
                        format_text($definition, $definitionformat, null, $peerreview->course->id), $levelid);
            }
 ...
 */
class peerreview_rubric_grid_assessment_form extends peerreview_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $numofcolumns = array_reduce($levelcounts, 'peerreview::lcm', 1);

        $mform->addElement('header', 'rubric-grid-wrapper', get_string('layoutgrid', 'peerreviewform_rubric'));

        $mform->addElement('html', '<table class="rubric-grid">' . "\n");
        $mform->addElement('html', '<th class="header">' . get_string('criteria', 'peerreviewform_rubric') . '</th>');
        $mform->addElement('html', '<th class="header" colspan="'.$numofcolumns.'">'.get_string('levels', 'peerreviewform_rubric').'</th>');
