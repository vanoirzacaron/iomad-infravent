...
/**
 * Provides the class {@link peerreviewform_rubric\privacy\provider}
 *
...
namespace peerreviewform_rubric\privacy;
...
class provider implements \core_privacy\local\metadata\null_provider, \mod_peerreview\privacy\peerreviewform_provider {
    ...
    public static function export_assessment_form(\stdClass $user, \context $context, array $subcontext, int $assessmentid) {
        ...
        $sql = "SELECT r.id, r.peerreviewid, r.description, r.descriptionformat,
                  ...
                  JOIN {peerreviewform_rubric} r ON r.peerreviewid = w.id
                  JOIN {peerreviewform_rubric_levels} rl ON rl.dimensionid = r.id
             LEFT JOIN {peerreview_grades} wg ON wg.strategy = :strategy AND wg.dimensionid = r.id AND wg.assessmentid = :assessmentid
        ...
        foreach ($rs as $record) {
            if (empty($criteria[$record->id])) {
                $criteria[$record->id] = (object) [
                    'description' => $writer->rewrite_pluginfile_urls($subcontext, 'peerreviewform_rubric', 'description',
        ...
        if ($hasdata) {
            ...
            $layout = $DB->get_field('peerreviewform_rubric_config', 'layout', ['peerreviewid' => $peerreviewid]);

            foreach (array_keys($criteria) as $dimensionid) {
                $writer->export_area_files($subcontext, 'peerreviewform_rubric', 'description', $dimensionid);
            }

            $writer->export_data($subcontext, $data);
            $writer->export_metadata($subcontext, 'layout', $layout, get_string('layout', 'peerreviewform_rubric'));
        }
    }
}
