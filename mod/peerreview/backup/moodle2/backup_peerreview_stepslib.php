...
/**
 * Defines all the backup steps that will be used by {@link backup_peerreview_activity_task}
 *
...
/**
 * Defines the complete peerreview structure for backup, with file and id annotations
 *
 * @link http://docs.moodle.org/dev/peerreview for XML structure diagram
 */
class backup_peerreview_activity_structure_step extends backup_activity_structure_step {

    /**
     * Defines the structure of the 'peerreview' element inside the peerreview.xml file
     *
    ...
    protected function define_structure() {
        ...
        $peerreview = new backup_nested_element('peerreview', array('id'), array(
        ...
        $this->add_subplugin_structure('peerreviewform', $peerreview, true);

        // grading evaluations data
        $this->add_subplugin_structure('peerrevieweval', $peerreview, true);
        ...
        $this->add_subplugin_structure('peerreviewform', $referenceassessment, true);
        ...
        $this->add_subplugin_structure('peerreviewform', $exampleassessment, true);
        ...
        $this->add_subplugin_structure('peerreviewform', $assessment, true);
        ...
        ////////////////////////////////////////////////////////////////////////
        $peerreview->add_child($examplesubmissions);
        ...
        $peerreview->set_source_table('peerreview', array('id' => backup::VAR_ACTIVITYID));

        $examplesubmission->set_source_sql("
            SELECT *
              FROM {peerreview_submissions}
             WHERE peerreviewid = ? AND example = 1",
        ...
        if ($userinfo) {
            ...
            $submission->set_source_sql("
                SELECT *
                  FROM {peerreview_submissions}
                 WHERE peerreviewid = ? AND example = 0",
                 array(backup::VAR_PARENTID));  // must use SQL here, for the same reason as above

            $assessment->set_source_table('peerreview_assessments', array('submissionid' => backup::VAR_PARENTID));

            $aggregation->set_source_table('peerreview_aggregations', array('peerreviewid' => backup::VAR_PARENTID));
        }
        ...
        $peerreview->annotate_files('mod_peerreview', 'intro', null); // no itemid used
        $peerreview->annotate_files('mod_peerreview', 'instructauthors', null); // no itemid used
        $peerreview->annotate_files('mod_peerreview', 'instructreviewers', null); // no itemid used
        $peerreview->annotate_files('mod_peerreview', 'conclusion', null); // no itemid used

        $examplesubmission->annotate_files('mod_peerreview', 'submission_content', 'id');
        $examplesubmission->annotate_files('mod_peerreview', 'submission_attachment', 'id');

        $referenceassessment->annotate_files('mod_peerreview', 'overallfeedback_content', 'id');
        $referenceassessment->annotate_files('mod_peerreview', 'overallfeedback_attachment', 'id');

        $exampleassessment->annotate_files('mod_peerreview', 'overallfeedback_content', 'id');
        $exampleassessment->annotate_files('mod_peerreview', 'overallfeedback_attachment', 'id');

        $submission->annotate_files('mod_peerreview', 'submission_content', 'id');
        $submission->annotate_files('mod_peerreview', 'submission_attachment', 'id');

        $assessment->annotate_files('mod_peerreview', 'overallfeedback_content', 'id');
        $assessment->annotate_files('mod_peerreview', 'overallfeedback_attachment', 'id');

        // return the root element (peerreview), wrapped into standard activity structure
