...
/**
 * Steps definitions related to peerreviewallocation_manual.
 *
 * @package    peerreviewallocation_manual
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_peerreviewallocation_manual extends behat_base {
    /**
     * Manually adds a reviewer for peerreview participant.
     *
     * This step should start on manual allocation page.
     *
     * @When /^I add a reviewer "(?P<reviewer_name_string>(?:[^"]|\\")*)" for peerreview participant "(?P<participant_name_string>(?:[^"]|\\")*)"$/
     * @param string $reviewername
     * @param string $participantname
     */
    public function i_add_a_reviewer_for_peerreview_participant($reviewername, $participantname) {
        ...
        try {
            ...
            $this->find_button(get_string('showallparticipants', 'peerreviewallocation_manual'))->press();
            $selectnode = $this->find('xpath', $xpathselect);
        }
        ...
        $allocatedtext = behat_context_helper::escape(get_string('allocationadded', 'peerreviewallocation_manual'));
        $this->find('xpath', "//*[contains(.,$allocatedtext)]");
    }

    /**
     * Manually allocates multiple reviewers in peerreview.
     *
     * @When /^I allocate submissions in peerreview "(?P<peerreview_name_string>(?:[^"]|\\")*)" as:$/
     * @When /^I allocate submissions in peerreview "(?P<peerreview_name_string>(?:[^"]|\\")*)" as:"$/
     ...
     */
    public function i_allocate_submissions_in_peerreview_as($peerreviewname, TableNode $table) {
        $this->execute("behat_navigation::go_to_breadcrumb_location", $peerreviewname);
        $this->execute('behat_navigation::i_navigate_to_in_current_page_administration',
            get_string('submissionsallocation', 'peerreview'));
        $rows = $table->getRows();
        ...
        for ($i = 1; $i < count($rows); $i++) {
            $this->execute(
                'behat_peerreviewallocation_manual::i_add_a_reviewer_for_peerreview_participant',
                [
                    $rows[$i][$reviewer],
                    $rows[$i][$participant],
                ]
