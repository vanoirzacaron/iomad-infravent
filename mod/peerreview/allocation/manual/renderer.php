 ...
 */
class peerreviewallocation_manual_renderer extends mod_peerreview_renderer  {
     ...
     *
     * @param peerreviewallocation_manual_allocations $data to be displayed
     * @return string html code
     */
    protected function render_peerreviewallocation_manual_allocations(peerreviewallocation_manual_allocations $data) {

        $this->peerreview     = $data->peerreview;
        ...
        $selfassessment     = $data->selfassessment;    // bool is the self-assessment allowed in this peerreview?
        ...
        $table->head        = array(get_string('participantreviewedby', 'peerreview'),
                                    get_string('participant', 'peerreview'),
                                    get_string('participantrevierof', 'peerreview'));
    ...
    /**
     * Returns information about the peerreview participant
     *
     * @return string HTML code
     */
    protected function helper_participant(stdclass $allocation, array $userinfo) {
        ...
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nosubmissionfound', 'peerreview'), 'info');
        } else {
            $link = $this->peerreview->submission_url($allocation->submissionid);
            ...
            if (is_null($allocation->submissiongrade)) {
                $o .= $this->output->container(get_string('nogradeyet', 'peerreview'), array('grade', 'missing'));
            } else {
                $o .= $this->output->container(get_string('alreadygraded', 'peerreview'), array('grade', 'missing'));
            }
        }
    ...
    protected function helper_reviewers_of_participant(stdclass $allocation, array $userinfo, array $reviewers, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nothingtoreview', 'peerreview'), 'info');
        } else {
            $exclude = array();
            if (! $selfassessment) {
            ...
            if ($options) {
                ...
                $select = new single_select($handler, 'by', $options, '', array(''=>get_string('chooseuser', 'peerreview')), 'addreviewof' . $allocation->userid);
                $select->set_label(get_string('addreviewer', 'peerreviewallocation_manual'));
    ...
    protected function helper_reviewees_of_participant(stdclass $allocation, array $userinfo, array $authors, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('withoutsubmission', 'peerreview'), 'info');
        }
        $exclude = array();
        if (! $selfassessment) {
            $exclude[$allocation->userid] = true;
            $o .= $this->output->container(get_string('selfassessmentdisabled', 'peerreview'), 'info');
        }
        // todo add an option to exclude users without own submission
        ...
        if ($options) {
            ...
            $select = new single_select($handler, 'of', $options, '', array(''=>get_string('chooseuser', 'peerreview')), 'addreviewby' . $allocation->userid);
            $select->set_label(get_string('addreviewee', 'peerreviewallocation_manual'));
            $o .= $this->output->render($select);
        } else {
            $o .= $this->output->container(get_string('nothingtoreview', 'peerreview'), 'info');
        }
