 ...
 *
 * @package    peerreviewallocation_random
 ...
 * Allocates the submissions randomly
 */
class peerreview_random_allocator implements peerreview_allocator {
    ...
    public function init() {
        ...
        $this->mform = new peerreview_random_allocator_form($PAGE->url, $customdata);
     ...
     *
     * @param peerreview_random_allocator_setting $setting
     * @param peerreview_allocation_result allocation result logger
     */
    public function execute(peerreview_random_allocator_setting $settings, peerreview_allocation_result $result) {

        $authors        = $this->peerreview->get_potential_authors();
        $authors        = $this->peerreview->get_grouped($authors);
        $reviewers      = $this->peerreview->get_potential_reviewers(!$settings->assesswosubmission);
        $reviewers      = $this->peerreview->get_grouped($reviewers);
        $assessments    = $this->peerreview->get_all_assessments();
        ...
        if ($settings->numofreviews) {
            ...
            $result->log(get_string('numofrandomlyallocatedsubmissions', 'peerreviewallocation_random', count($randomallocations)));
        ...
        if ($settings->addselfassessment) {
            ...
            $result->log(get_string('numofselfallocatedsubmissions', 'peerreviewallocation_random', count($selfallocations)));
            unset($selfallocations);
        }
        if (empty($newallocations)) {
            $result->log(get_string('noallocationtoadd', 'peerreviewallocation_random'), 'info');
        } else {
            ...
            foreach ($newallocations as $newallocation) {
                ...
                if (isset($allreviewers[$reviewerid])) {
                    ...
                    if (!$allreviewersreloaded) {
                        $allreviewers = $this->peerreview->get_potential_reviewers(false);
                ...
                if (in_array($newallocation, $newnonexistingallocations)) {
                    $result->log(get_string('allocationaddeddetail', 'peerreviewallocation_random', $a), 'ok', 1);
                } else {
                    $result->log(get_string('allocationreuseddetail', 'peerreviewallocation_random', $a), 'ok', 1);
                }
            }
        }
        if ($settings->removecurrent) {
            ...
            // by reviewer
            $result->log(get_string('numofdeallocatedassessment', 'peerreviewallocation_random', count($delassessments)), 'info');
            foreach ($delassessments as $delassessmentkey => $delassessmentid) {
                ...
                if (!is_null($assessments[$delassessmentid]->grade)) {
                    $result->log(get_string('allocationdeallocategraded', 'peerreviewallocation_random', $a), 'error', 1);
                    unset($delassessments[$delassessmentkey]);
                } else {
                    $result->log(get_string('assessmentdeleteddetail', 'peerreviewallocation_random', $a), 'info', 1);
                }
            }
            $this->peerreview->delete_assessment($delassessments);
        }
        $result->set_status(peerreview_allocation_result::STATUS_EXECUTED);
    }
    ...
    public function ui() {
        ...
        if ($m == self::MSG_SUCCESS) {
            $message->set_text(get_string('randomallocationdone', 'peerreviewallocation_random'));
            $message->set_type(peerreview_message::TYPE_OK);
        }
        ...
        $gmode = groups_get_activity_groupmode($this->peerreview->cm, $this->peerreview->course);
        if (VISIBLEGROUPS == $gmode or SEPARATEGROUPS == $gmode) {
            $users = $this->peerreview->get_potential_authors() + $this->peerreview->get_potential_reviewers();
            $users = $this->peerreview->get_grouped($users);
            if (isset($users[0])) {
                ...
                if (!empty($nogroupusers)) {
                    ...
                    $a = implode(', ', $list);
                    $out .= $output->box(get_string('nogroupusers', 'peerreviewallocation_random', $a), 'generalbox warning nogroupusers');
                }
            }
        }

        // TODO $out .= $output->heading(get_string('stats', 'peerreviewallocation_random'));
     ...
     *
     * @param array $authors grouped of {@see peerreview::get_potential_authors()}
     * @param array $reviewers grouped by {@see peerreview::get_potential_reviewers()}
     * @param array $assessments as returned by {@see peerreview::get_all_assessments()}
    ...
    protected function add_new_allocations(array $newallocations, array $dataauthors, array $datareviewers) {
        ...
        $submissions    = $this->peerreview->get_submissions($authorids);
        $submissions    = $this->index_submissions_by_authors($submissions);
        foreach ($newallocations as $newallocation) {
            $reviewerid = key($newallocation);
            $authorid = current($newallocation);
            if (!isset($submissions[$authorid])) {
                throw new moodle_exception('unabletoallocateauthorwithoutsubmission', 'peerreview');
            }
            $submission = $submissions[$authorid];
            $status = $this->peerreview->add_allocation($submission, $reviewerid, 1, true);   // todo configurable weight?
            if (peerreview::ALLOCATION_EXISTS == $status) {
        ...
        if (is_array($submissions)) {
            foreach ($submissions as $submissionid => $submission) {
                if (isset($byauthor[$submission->authorid])) {
                    throw new moodle_exception('moresubmissionsbyauthor', 'peerreview');
                }
     ...
     * @param peerreview_allocation_result $result allocation result logger
    ...
    protected function random_allocation($authors, $reviewers, $assessments, $result, array $options) {
        ...
        if (peerreview_random_allocator_setting::NUMPER_SUBMISSION == $numper) {
            // circles are authors, squares are reviewers
            $result->log(get_string('resultnumperauthor', 'peerreviewallocation_random', $numofreviews), 'info');
            $allcircles = $authors;
            $allsquares = $reviewers;
            // get current workload
            list($circlelinks, $squarelinks) = $this->convert_assessments_to_links($assessments);
        } elseif (peerreview_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            $result->log(get_string('resultnumperreviewer', 'peerreviewallocation_random', $numofreviews), 'info');
            $allcircles = $reviewers;
            $allsquares = $authors;
            // get current workload
            list($squarelinks, $circlelinks) = $this->convert_assessments_to_links($assessments);
        } else {
            throw new moodle_exception('unknownusertypepassed', 'peerreview');
        }
                    ...
                    while ($keeptrying && (count($circlelinks[$circleid]) < $requiredreviews)) {
                        ...
                        if (NOGROUPS == $gmode) {
                            if (in_array(0, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeers', 'peerreviewallocation_random'), 'error', 1);
                                break;
                            }
                            $targetgroup = 0;
                            ...
                            if (in_array($circlegroupid, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeersingroup', 'peerreviewallocation_random'), 'error', 1);
                                break;
                            }
                        ...
                        if ($targetgroup === false) {
                            $keeptrying = false;
                            $result->log(get_string('resultnotenoughpeers', 'peerreviewallocation_random'), 'error', 1);
                            break;
                        }
        ...
        if (peerreview_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            foreach ($circlelinks as $circleid => $squares) {
                foreach ($squares as $squareid) {
     ...
     *
     * @param array $assessments array of assessments as returned by {@link peerreview::get_all_assessments()}
 ...
 */
class peerreview_random_allocator_setting {
     ...
     * @return peerreview_random_allocator_setting
     */
