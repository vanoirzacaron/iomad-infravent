...
class learner extends block_base {
    ...
    private function get_table_grade($userid, $coursetable) {
        global $DB;
        $sql = "SELECT c.tempid, (gg.finalgrade / gi.grademax * 100) grades
                  FROM {{$coursetable}} c
                  JOIN {grade_items} gi ON c.tempid = gi.courseid
    ...
    public function get_table_data($filter) {
        ...
        $coursetable = utility::create_temp_table('tmp_sql_c', $usercourses);

        $sql = "SELECT c.id, c.fullname
                  FROM {{$coursetable}} uc
                  JOIN {course} c ON uc.tempid = c.id
                 WHERE " . $DB->sql_like('c.fullname', ':fullname', false);
        $params = [
            'userid' => $userid,
            'fullname' => "%$search%"
        ];
