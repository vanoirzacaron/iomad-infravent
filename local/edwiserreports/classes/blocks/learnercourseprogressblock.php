...
class learnercourseprogressblock extends block_base {
    ...
    public function get_filter() {
        ...
        return $OUTPUT->render_from_template('local_edwiserreports/blocks/learnercourseprogressblockfilters', [
    ...
    public function get_data($filter = false) {
        ...
        if ($course === 0) { // Course is selected in dropdown.
            ...
            if (!empty($records)) {
                foreach ($filteredcourses as $record) {
                    $labels[] = format_string($courses[$record->id]->fullname, true, ['context' => \context_system::instance()]);
            ...
            if (!empty($record)) {
                // Completed.
                $labels[] = get_string('completed-y', 'local_edwiserreports');
                $progress[] = (int)$record->total;

                // Incomplete.
                $labels[] = get_string('completed-n', 'local_edwiserreports');
        ...
        return [
            ...
            'progress' => $progress,
            'tooltip' => [
                'single' => get_string('activity', 'local_edwiserreports'),
                'plural' => get_string('activities', 'local_edwiserreports')
            ]
        ];
    }
}
