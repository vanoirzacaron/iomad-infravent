...
class utility {
    ...
    public static function get_active_users_data($data) {
        global $CFG;
        $activeusersblock = new \local_edwiserreports\blocks\activeusersblock($data);
    ...
    public static function get_certificates_data($data) {
        $certificatesblock = new \local_edwiserreports\blocks\certificatesblock();
        if (isset($data->certificateid)) {
            $cohortid = isset($data->cohortid) ? $data->cohortid : false;
            return $certificatesblock->get_issued_users(
                $data->certificateid,
                $cohortid
            );
        }
    ...
    public static function get_liveusers_data() {
        $liveusers = new \local_edwiserreports\blocks\liveusersblock();
    ...
    public static function get_siteaccess_data() {
        $siteaccess = new \local_edwiserreports\blocks\siteaccessblock();
    ...
    public static function get_todaysactivity_data($data) {
        $todaysactivityblock = new \local_edwiserreports\blocks\todaysactivityblock();
    ...
    public static function get_inactiveusers_data($data) {
        $inactiveusers = new \local_edwiserreports\blocks\inactiveusersblock();
    ...
    public static function get_completion_data($data) {
        ...
        return $completionblock->get_completion_data($data);
    }
    ...
    public static function get_grade_graph_data($data) {
        $grade = new \local_edwiserreports\blocks\gradeblock();
    ...
    public static function get_courses($all = false) {
        ...
        //     $enrolledstudents = self::get_enrolled_students($course->id);
        //     if ($course->id == 1 || empty($enrolledstudents)) {
    ...
    public static function get_scheduled_email_details($data) {
        ...
        } else if (!is_array($emaildata)) { // If data is not an array.
            $response->error = true;
     ...
     */
    public static function get_customreport_course_selectors() {
        // Get all courses.
     ...
     * Added extra optional date parameter to get users according to date filter
     * This is mainly used in summary card data
     * @param  Integer     $courseid Course Id
     ...
     * @return Array                 Array of users
     */
    public static function get_enrolled_students(
        $courseid,
        $context = false,
        $cohortid = 0,
        $groupid = 0,
        ...
        $enrolmentsql = '';
        $enrolmentcond = '';
        // Get userlist after applying date filter.
        if (isset($date['starttime'])) {
            // list($starttime, $endtime) = $this->get_date_range($filters->enrolment);
            $params['starttime'] = floor($date['starttime'] / 86400);
            $params['endtime'] = floor($date['endtime'] / 86400);
            $params['course'] = $courseid;
            $enrolmentsql = 'JOIN {user_enrolments} ue ON ue.userid = u.id
                            JOIN {enrol} e ON (e.id = ue.enrolid AND e.courseid = :course)';
            $enrolmentcond = 'AND floor(ue.timecreated / 86400) between :starttime AND :endtime ';
        }
     ...
     * @param  Object $reportname Report name.
     * @return Array
     */
    public static function get_report_capability($reportname) {
        $context = context_system::instance();

        $capabilitychoices = array();
        foreach ($context->get_capabilities() as $cap) {
            if (strpos($cap->name, 'report/edwiserreports_' . $reportname . ':') === false) {
                continue;
            }
            if (!has_capability($cap->name, $context) && !can_view_block($cap->name)) {
                continue;
            }
        ...
        // Adding additional capability for learner course progress report page.
        $capname = 'report/edwiserreports_learner:editadvance';
        if ($reportname === 'learnercourseprogress' && (has_capability($capname, $context) || can_view_block($capname))) {
            $capabilitychoices['report/edwiserreports_learner:editadvance'] = get_string(
                'learnereditadvance',
                'local_edwiserreports'
            );
            $capabilitychoices['report/edwiserreports_learner:view'] = get_string('learnerview', 'local_edwiserreports');
        }
    ...
    public static function get_active_theme() {
        $theme = get_config('local_edwiserreports', 'theme');
        $theme = $theme === false ? 0 : $theme;

        if (isset(LOCAL_EDWISERREPORTS_COLOR_THEMES[$theme])) {
            return $theme;
        }
        return 0;
    }
