...
class mysql_old_logs {
    ...
    private function get_logs_count($course, $user, $mintime, $timeslot) {
        ...
        if ($records->valid()) {
            $previous = $records->current()->timecreated;
            foreach ($records as $record) {
                $timespent = $record->timecreated - $previous;
                $previous = $record->timecreated;
                if ($timespent < $timeslot && $timespent > 0) {
                    $count++;
                }
            }
        }
    ...
    private function process_users_old_logs($courseid, $userid, $mintime, $timeslot, $percent, $increment, $progress) {
        ...
        while (true) {
            ...
            $previous = $logs->current()->timestart;
            $offset += LIMIT_NUM;
            foreach ($logs as $log) {
                $timespent = $log->timestart - $previous;
                $previous = $log->timestart;
                if ($timespent == null ||
                    $timespent < 1 ||
                    $timespent > $timeslot
                ) {
     ...
     * @param int   $timeslot Time slot between clicks.
     */
    public function process_old_logs($courses, $mintime, $timeslot) {
        global $DB;
        preg_match('@[0-9]+\.[0-9]+\.[0-9]+@', $DB->get_server_info()['version'], $version);
        echo "<br><strong>" . get_string('oldlogmysqlwarning', 'local_edwiserreports', $version[0]) . "</strong>";
